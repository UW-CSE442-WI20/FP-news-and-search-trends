...
},{}],"brython.js":[function(require,module,exports) {
...
// implementation [3, 8, 7, 'final', 0]
// version compiled from commented, indented source files at
// github.com/brython-dev/brython
var __BRYTHON__ = __BRYTHON__ || {};

(function ($B) {
  $B.isWebWorker = 'undefined' !== typeof WorkerGlobalScope && "function" === typeof importScripts && navigator instanceof WorkerNavigator;
  ...
  $B.__setattr__ = function (attr, value) {
    if (['debug', 'stdout', 'stderr'].indexOf(attr) > -1) {
      ...
      throw $B.builtins.AttributeError.$factory('__BRYTHON__ object has no attribute ' + attr);
    }
  };
...
(function ($B) {
  ...
  var add_identnode = $B.parser.add_identnode = function (parent, insert_at, name, val) {
    ...
    if (insert_at === -1) parent.add(new_node);else parent.insert(insert_at, new_node);
    assign.tree[1] = val;
    return new_node;
  };
  ...
  var $add_yield_from_code = $B.parser.$add_yield_from_code = function (yield_ctx) {
    ...
    for (attr in repl) {
      replace_with = replace_with.replace(new RegExp("\\b" + attr + "\\b", 'g'), repl[attr]);
  ...
  var $Node = $B.parser.$Node = function (type) {
    ...
    this.transform = function (rank) {
      ...
      if (this.type === 'module') {
        ...
        if (this.C && this.C.tree !== undefined && this.C.tree[0].type == "generator") {
          var def_node = this,
              def_ctx = def_node.C.tree[0];
          var blocks = [],
          ...
          while (true) {
            ...
            if (node === undefined || node.id == '__builtins__') {
              break;
            }
          }
          ...
          var g = $B.$BRgenerator(def_ctx.name, blocks, def_ctx.id, def_node),
              block_id = parent.id.replace(/\./g, '_'),
              name = def_ctx.decorated ? def_ctx.alias : def_ctx.name + def_ctx.num,
              res = 'var ' + def_ctx.name + def_ctx.num + ' = ' + '$locals_' + block_id + '["' + def_ctx.name + '"] = $B.genfunc("' + def_ctx.name + '", ' + blocks + ',[' + g + '],' + def_ctx.default_str + ')';
  ...
  var $YieldFromMarkerNode = $B.parser.$YieldFromMarkerNode = function (params) {
    ...
    this.transform = function (rank) {
      add_identnode(this.parent, rank, params.iter_name, new $JSCode('$B.$iter(' + params.yield_expr.tree[0].to_js() + ')'));
  ...
  var $AnnotationCtx = $B.parser.$AnnotationCtx = function (C) {
    ...
    if (scope.ntype == "def" && C.tree && C.tree.length > 0 && C.tree[0].type == "id") {
      var name = C.tree[0].value;

      if (scope.globals && scope.globals.has(name) > -1) {
        $_SyntaxError(C, ["annotated name '" + name + "' can't be global"]);
      }
  ...
  var $AssignCtx = $B.parser.$AssignCtx = function (C, expression) {
    ...
    } else if (C.type == 'list_or_tuple' || C.type == 'expr' && C.tree[0].type == 'list_or_tuple') {
      if (C.type == 'expr') {
        C = C.tree[0];
      }
    ...
    this.transform = function (node, rank) {
      ...
      switch (left.type) {
        case 'expr':
          if (left.tree.length > 1) {
            ...
            if (scope.globals && scope.globals.has(name)) {} else {
      ...
      if (right.type == 'list' || right.type == 'tuple' || right.type == 'expr' && right.tree.length > 1) {
      ...
      if (right_items !== null) {
        ...
        var new_node = $NodeJS('var ' + rname + ' = ' + '$B.$getattr($B.$iter(' + right.to_js() + '), "__next__");');
        new_node.line_num = node.line_num;
        node.parent.insert(rank++, new_node);
        node.parent.insert(rank++, $NodeJS('var ' + rlname + '=[], $pos=0;' + 'while(1){' + 'try{' + rlname + '[$pos++] = ' + rname + '()' + '}catch(err){' + 'break' + '}' + '}'));
        var packed = null;
        ...
        for (var i = 0; i < left_items.length; i++) {
          ...
          if (expr.type == 'packed' || expr.type == 'expr' && expr.tree[0].type == 'packed') {
            packed = i;
            min_length--;
            break;
          }
        ...
        node.parent.insert(rank++, $NodeJS('if(' + rlname + '.length<' + min_length + '){' + 'throw ValueError.$factory(' + '"need more than " +' + rlname + '.length + " value" + (' + rlname + '.length > 1 ?' + ' "s" : "") + " to unpack")}'));

        if (packed == null) {
          node.parent.insert(rank++, $NodeJS('if(' + rlname + '.length>' + min_length + '){' + 'throw ValueError.$factory(' + '"too many values to unpack ' + '(expected ' + left_items.length + ')"' + ')' + '}'));
        }
        ...
        left_items.forEach(function (left_item, i) {
          ...
          if (packed == null || i < packed) {
            ...
            js += '.slice(' + i + ',' + rlname + '.length-' + (left_items.length - i - 1) + ')';
          } else {
    ...
    this.to_js = function () {
      ...
      if (left.type == 'attribute' || left.type == 'sub') {
        ...
        if (right.type == 'expr' && right.tree[0] !== undefined && right.tree[0].type == 'call' && ('eval' == right.tree[0].func.value || 'exec' == right.tree[0].func.value)) {
          res += 'var ' + $var + ' = ' + right_js + ';\n';
          rvar = $var;
        } else if (right.type == 'expr' && right.tree[0] !== undefined && right.tree[0].type == 'sub') {
          res += 'var ' + $var + ' = ' + right_js + ';\n';
          rvar = $var;
        } else {
          rvar = right_js;
        ...
        if (left.type == 'sub') {
          ...
          if (left.tree.length == 1) {
            res += '$B.set_list_key(' + temp + ',' + (left.tree[0].to_js() + '' || 'null') + ',' + right.to_js() + ')';
          } else if (left.tree.length == 2) {
            res += '$B.set_list_slice(' + temp + ',' + (left.tree[0].to_js() + '' || 'null') + ',' + (left.tree[1].to_js() + '' || 'null') + ',' + right.to_js() + ')';
          } else if (left.tree.length == 3) {
            res += '$B.set_list_slice_step(' + temp + ',' + (left.tree[0].to_js() + '' || 'null') + ',' + (left.tree[1].to_js() + '' || 'null') + ',' + (left.tree[2].to_js() + '' || 'null') + ',' + right.to_js() + ')';
          }
          ...
          if (left.tree.length == 1) {
            res += '$B.$setitem(' + left.value.to_js() + ',' + left.tree[0].to_js() + ',' + right_js + ')};None;';
          } else {
            left.func = 'setitem';
            res += left.to_js();
            res = res.substr(0, res.length - 1);
  ...
  var $AttrCtx = $B.parser.$AttrCtx = function (C) {
    ...
    this.toString = function () {
      return '(attr) ' + this.value + '.' + this.name;
    };
      ...
      if (this.func == "setattr" && this.value.type == "id") {
        ...
        if (scope.ntype == "def") {
          if (parent.ntype == "class") {
            ...
            if (this.value.value == params[0] && parent.C && parent.C.tree[0].args === undefined) {
              this.assign_self = true;
              return [js + ".__class__ && " + js + ".__dict__ && !" + js + ".__class__.$has_setattr && ! " + js + ".$is_class ? _b_.dict.$setitem(" + js + ".__dict__, '" + this.name + "', ", ") : $B.$setattr(" + js + ', "' + this.name + '", '];
            }
          }
        }
      }
  ...
  var $AugmentedAssignCtx = $B.parser.$AugmentedAssignCtx = function (C, op) {
    ...
    if (C.type == 'expr') {
      ...
      if (assigned.type == 'id') {
        ...
        if (noassign[name] === true) {
          $_SyntaxError(C, ["cannot assign to keyword"]);
        } else if ((scope.ntype == 'def' || scope.ntype == 'generator') && scope.binding[name] === undefined) {
          if (scope.globals === undefined || !scope.globals.has(name)) {
    ...
    this.transform = function (node, rank) {
      ...
      if (left_is_id) {
        var left_bound_to_int = this.tree[0].tree[0].bindingType(this.scope) == "int";
        this.tree[0].tree[0].augm_assign = true;

        if ($B.debug > 0) {
          var check_node = $NodeJS('if(' + this.tree[0].to_js() + ' === undefined){throw NameError.$factory("name \'' + this.tree[0].tree[0].value + '\' is not defined")}');
        ...
        case '/=':
          if (left_is_id) {
            ...
            switch (scope.ntype) {
              ...
              case 'def':
              case 'generator':
                if (scope.globals && scope.globals.has(C.tree[0].value)) {
                  prefix = global_ns;
                } else {
                  prefix = '$locals';
                }
      ...
      if (C.tree[0].type == "id") {
        var binding_scope = C.tree[0].firstBindingScopeId();

        if (binding_scope) {
          left = "$locals_" + binding_scope.replace(/\./g, '_') + '["' + C.tree[0].value + '"]';
        }
      }
      ...
      if (prefix) {
        ...
        js += ' && Number.isSafeInteger(' + left + op1 + right + ')){' + (right_is_int ? '(' : '(typeof $temp == "number" && ') + 'typeof ' + left1 + ' == "number") ? ';
        js += left + op + right;
        js += ' : ' + left + ' = new Number(' + left + op1 + (right_is_int ? right : right + '.valueOf()') + ')}';
        new $NodeJSCtx(new_node, js);
        parent.insert(rank + offset, new_node);
        offset++;
      }
      ...
      if (C.tree[0].type == 'sub' && ('+=' == op || '-=' == op || '*=' == op) && C.tree[0].tree.length == 1) {
        var js1 = '$B.augm_item_' + aaops[op] + '(' + C.tree[0].value.to_js() + ',' + C.tree[0].tree[0].to_js() + ',' + right + ');None;';
        var new_node = new $Node();
      ...
      assign2.tree.push($NodeJS('$B.$getattr(' + C.to_js() + ',"' + func + '")(' + right + ')'));
  ...
  var $BreakCtx = $B.parser.$BreakCtx = function (C) {
    ...
    this.to_js = function () {
      this.js_processed = true;
      var scope = $get_scope(this);
      var res = ';$locals_' + scope.id.replace(/\./g, '_') + '["$no_break' + this.loop_ctx.loop_num + '"] = false';
  ...
  var $CallCtx = $B.parser.$CallCtx = function (C) {
    ...
    if (this.func && this.func.type == "attribute" && this.func.name == "wait" && this.func.value.type == "id" && this.func.value.value == "time") {
    ...
    this.to_js = function () {
      ...
      if (this.func !== undefined) {
          ...
          case '$$super':
            if (this.tree.length == 0) {
              ...
              if (scope.ntype == 'def' || scope.ntype == 'generator') {
                var def_scope = $get_scope(scope.C.tree[0]);

                if (def_scope.ntype == 'class') {
                  new $IdCtx(this, def_scope.C.tree[0].name);
                }
              }
            }
        ...
        if (dstar_args.length) {
          kw_args_str = '{$nat:"kw",kw:[' + kw_args_str + ',' + dstar_args.join(', ') + ']}';
        } else if (kw_args_str != '{}') {
        ...
        if (star_args && kw_args_str) {
          ...
          if (args_str && kw_args_str) {
            args_str += ',' + kw_args_str;
          } else if (!args_str) {
            args_str = kw_args_str;
          }
        }
        ...
        if (this.tree.length > -1 && this.func.type == 'id' && this.func.is_builtin) {
  ...
  var $ClassCtx = $B.parser.$ClassCtx = function (C) {
    ...
    this.set_name = function (name) {
      ...
      while (parent_block.C && 'def' != parent_block.C.tree[0].type && 'generator' != parent_block.C.tree[0].type) {
        parent_block = parent_block.parent;
      }
    ...
    this.transform = function (node, rank) {
      ...
      var indent = '\n' + ' '.repeat(node.indent + 12),
          ...
          js = 'var ' + local_ns + ' = {' + '__annotations__: _b_.dict.$factory()}, ' + indent + '$locals = ' + local_ns + ', ' + indent + '$local_name = "' + local_ns + '"';
      ...
      var js = ' '.repeat(node.indent + 4) + '$locals.$name = "' + this.name + '"' + indent + '$locals.$line_info = "' + node.line_num + ',' + this.module + '";' + indent + 'var $top_frame = [$local_name, $locals,' + '"' + global_scope.id + '", ' + global_ns + ']' + indent + '$locals.$f_trace = $B.enter_frame($top_frame);' + indent + 'if($locals.$f_trace !== _b_.None){' + '$locals.$f_trace = $B.trace_line()}';
      node.insert(1, $NodeJS(js));
      node.add($NodeJS('if($locals.$f_trace !== _b_.None){' + '$B.trace_return(_b_.None)}'));
      ...
      rank++;
      node.parent.insert(rank + 1, $NodeJS('$' + this.name + '_' + this.random + ".__module__ = " + module_name));
      var scope = $get_scope(this);
    ...
    this.transform = function (node, rank) {
      ...
      if (this.token == "while") {
        ...
        if ($B.last(node.children).C.tree[0].type != "return") {
          var js = '$locals.$line_info = "' + node.line_num + ',' + module + '";if($locals.$f_trace !== _b_.None){' + '$B.trace_line()};None;';
          node.add($NodeJS(js));
        }
  ...
  var $DecoratorCtx = $B.parser.$DecoratorCtx = function (C) {
    ...
    this.transform = function (node, rank) {
      ...
      while (1) {
        if (func_rank >= children.length) {
          $_SyntaxError(C, ['decorator expects function']);
        } else if (children[func_rank].C.type == 'node_js') {
          func_rank++;
    ...
    this.to_js = function () {
      ...
      this.decorators.forEach(function (decorator, i) {
        res.push('var ' + this.dec_ids[i] + ' = ' + $to_js(decorator) + ';');
      }, this);
      return res.join('');
    };
  };
  ...
  var $DefCtx = $B.parser.$DefCtx = function (C) {
    ...
    this.set_name = function (name) {
      ...
      if (this.scope.globals !== undefined && this.scope.globals.has(name)) {
    ...
    this.transform = function (node, rank) {
      ...
      while (global_scope.parent_block && global_scope.parent_block.id !== '__builtins__') {
        global_scope = global_scope.parent_block;
      }
      ...
      var local_ns = '$locals_' + this.id,
          h = '\n' + ' '.repeat(indent);
      js = 'var ' + local_ns + ' = {},' + h + '$local_name = "' + this.id + '",' + h + '$locals = ' + local_ns + ';';
      var new_node = new $Node();
      new_node.locals_def = true;
      new_node.func_node = node;
      new $NodeJSCtx(new_node, js);
      nodes.push(new_node);
      var enter_frame_nodes = [$NodeJS('$locals.$line_info = "' + node.line_num + ',' + this.module + '"'), $NodeJS('var $top_frame = [$local_name, $locals,' + '"' + global_scope.id + '", ' + global_ns + ', ' + name + ']'), $NodeJS('$locals.$f_trace = $B.enter_frame($top_frame)'), $NodeJS('var $stack_length = $B.frames_stack.length;')];

      if (this.async) {
        enter_frame_nodes.push($NodeJS("var $stack = " + "$B.frames_stack.slice()"));
      }
      ...
      js += ' = $B.args("' + this.name + '", ' + this.argcount + ', {' + this.slots.join(', ') + '}, ' + '[' + slot_list.join(', ') + '], arguments, $defaults, ' + this.other_args + ', ' + this.other_kw + ');';
      var new_node = new $Node();
      new $NodeJSCtx(new_node, js);
      ...
      if (this.other_args === null && this.other_kw === null && this.after_star.length == 0 && !has_end_pos) {
        ...
        if (this.type == 'generator') {
          if (this.args.length == 0) {
            test_node.add($NodeJS('//'));
          } else {
            this.args.forEach(function (arg) {
              test_node.add($NodeJS('$locals["' + arg + '"] = ' + arg));
            });
          }
        } else {
        ...
        else_node.add($NodeJS('else if($len > ' + pos_len + '){$B.wrong_nb_args("' + this.name + '", $len, ' + pos_len + ', [' + slot_list + '])}'));

        if (pos_len > 0) {
          else_node.add($NodeJS('else if($len + $nb_defaults < ' + pos_len + '){$B.wrong_nb_args("' + this.name + '", $len, ' + pos_len + ', [' + slot_list + '])}'));
          subelse_node = $NodeJS("else");
          else_node.add(subelse_node);

          if (this.type == 'generator') {
            this.args.forEach(function (arg) {
              subelse_node.add($NodeJS('$locals["' + arg + '"] = ' + arg));
            });
          } else {
          ...
          subelse_node.add($NodeJS("for(var i=$len; i < defparams.length" + ";i++){$locals[defparams[i]] = $defaults[defparams[i]]}"));
        }
      } else {
      ...
      if (is_method) {
        var class_name = scope.C.tree[0].name,
            ...
            class_ref = "$locals_" + class_block.id.replace(/\./g, '_') + '["' + class_name + '"]';
      ...
      if (this.type != "generator") {
        if (this.otherdefaults.length > 0) {
          ...
          node.parent.insert(rank + offset++, $NodeJS('    __defaults__ : ' + '$B.fast_tuple([' + def_names.join(', ') + ']),'));
        } else {
          node.parent.insert(rank + offset++, $NodeJS('    __defaults__ : ' + '_b_.None,'));
        }
        ...
        if (this.kwonlyargsdefaults.lengh > 0) {
          ...
          node.parent.insert(rank + offset++, $NodeJS('    __kwdefaults__ : ' + '$B.fast_tuple([' + def_names.join(', ') + ']),'));
        } else {
          node.parent.insert(rank + offset++, $NodeJS('    __kwdefaults__ : ' + '_b_.None,'));
        }
      }
      ...
      var h1 = ',' + h + ' '.repeat(4);
      var module = $get_module(this).module;
      js += h1 + 'co_filename:$locals_' + module.replace(/\./g, '_') + '["__file__"]' + h1 + 'co_firstlineno:' + node.line_num + h1 + 'co_flags:' + flags + h1 + 'co_freevars: [' + free_vars + ']' + h1 + 'co_kwonlyargcount:' + this.kwonlyargcount + h1 + 'co_name: "' + this.name + '"' + h1 + 'co_nlocals: ' + co_varnames.length + h1 + 'co_posonlyargcount: ' + (this.pos_only || 0) + h1 + 'co_varnames: $B.fast_tuple([' + co_varnames.join(', ') + ']' + h + ')}\n' + ' '.repeat(indent + 4) + '};';
      js += 'None;';
      node.parent.insert(rank + offset++, $NodeJS(js));
      ...
      if (this.type == "def") {
        ...
        node.parent.insert(rank + offset++, $NodeJS(this.func_name + " = " + this.name + '$' + this.num + '(' + this.default_str + ')'));
        node.parent.insert(rank + offset++, $NodeJS(func_name1 + ".$set_defaults = function(value){return " + func_name1 + " = " + this.name + "$" + this.num + "(value)}"));
      }

      if (this.type == 'def') {
        var parent = node;

        for (var pos = 0; pos < parent.children.length && parent.children[pos] !== $B.last(enter_frame_nodes); pos++) {}
        ...
        except_node.add($NodeJS('if($locals.$f_trace !== _b_.None){' + '$locals.$f_trace = $B.trace_exception()}'));
    ...
    this.to_js = function (func_name) {
      ...
      return "var " + this.name + '$' + this.num + ' = function($defaults){' + (this.async ? 'async ' : '') + 'function ' + this.name + this.num + '(' + this.params + ')';
    };
  };
  ...
  var $DelCtx = $B.parser.$DelCtx = function (C) {
    ...
    this.to_js = function () {
      ...
      if (this.tree[0].type == 'list_or_tuple') {
        ...
        this.tree = [];
        return res.join(';');
      } else if (this.tree[0].type == 'expr' && this.tree[0].tree[0].type == 'list_or_tuple') {
        ...
        switch (expr.type) {
          case 'id':
            ...
            if ((scope.ntype == "def" || scope.ntype == "generator") && scope.globals && scope.globals.has(expr.value)) {
              scope = scope.parent;

              while (scope.parent && scope.parent.id !== "__builtins__") {
                scope = scope.parent;
              }
            ...
            var res = '$B.$delete("' + expr.value + '"' + (is_global ? ', "global"' : '') + ');';
    ...
    this.unpack_dict = function (packed) {
      ...
      while (i < this.items.length) {
        ...
        if (item.type == "expr" && item.packed) {
          ...
          res = "[[" + item.to_js() + "," + this.items[i + 1].to_js() + "]]";
          i += 2;
        }
    ...
    this.to_js = function () {
      ...
      switch (this.real) {
        case 'dict':
          ...
          if (packed.length > 0) {
            return '_b_.dict.$factory(' + this.unpack_dict(packed) + ')' + $to_js(this.tree);
          }

          var res = [];

          for (var i = 0; i < this.items.length; i += 2) {
            res.push('[' + this.items[i].to_js() + ',' + this.items[i + 1].to_js() + ']');
          }

          return '_b_.dict.$factory([' + res.join(',') + '])' + $to_js(this.tree);

        case 'set_comp':
          return '_b_.set.$factory(' + $to_js(this.items) + ')' + $to_js(this.tree);

        case 'dict_comp':
          return '_b_.dict.$factory(' + $to_js(this.items) + ')' + $to_js(this.tree);
      }
  ...
  var $ExceptCtx = $B.parser.$ExceptCtx = function (C) {
    ...
    this.transform = function (node, rank) {
      ...
      if (last_child.C.tree && last_child.C.tree[0] && last_child.C.tree[0].type == "return") {} else {
    ...
    this.to_js = function () {
      ...
      if ($B.debug > 0) {
        var module = $get_module(this);
        lnum = '($locals.$line_info = "' + $get_node(this).line_num + ',' + module.id + '") && ';
      }
    ...
    this.to_js = function (arg) {
      ...
      if (this.assign) {
        ...
        if (scope.globals && scope.globals.has(this.assign.value)) {
          while (scope.parent_block && scope.parent_block.id !== "__builtins__") {
            scope = scope.parent_block;
          }
        } else if (scope.nonlocals && scope.nonlocals[this.assign.value]) {
          scope = scope.parent_block;
        }

        res = "($locals_" + scope.id.replace(/\./g, '_') + '["' + this.assign.value + '"] = ' + res + ')';
      }
  ...
  var $FloatCtx = $B.parser.$FloatCtx = function (C, value) {
    ...
    this.to_js = function () {
      ...
      if (/^\d+$/.exec(this.value) || /^\d+\.\d*$/.exec(this.value)) {
  ...
  var $ForExpr = $B.parser.$ForExpr = function (C) {
    ...
    this.transform = function (node, rank) {
      ...
      if (target.tree.length == 1 && !scope.blurred && target.expct != 'id' && iterable.type == 'expr' && iterable.tree[0].type == 'expr' && iterable.tree[0].tree[0].type == 'call') {
        var call = iterable.tree[0].tree[0];

        if (call.func.type == 'id') {
          var func_name = call.func.value;

          if (func_name == 'range' && call.tree.length < 3 && call.tree.length > 0) {
            $range = call;
          }
        }
      }
      ...
      if ($range && scope.ntype != 'generator') {
        ...
        while (1) {
          if (_scope.binding["range"]) {
            found.push(_scope.id);
          }

          if (_scope.parent_block) {
            _scope = _scope.parent_block;
          } else {
            break;
          }
        }
        ...
        if (range_is_builtin) {
          ...
          new $NodeJSCtx(test_range_node, 'if(' + call.func.to_js() + ' === $B.builtins.range)');
        }
        ...
        if ($range.tree.length == 1) {
          ...
          if (stop.tree[0].type == "int") {
            ...
            if (0 < stop < $B.max_int) {
              ...
              var for_node = $NodeJS("for (var " + varname + " = 0; " + varname + " < " + stop + "; " + varname + "++)");
        ...
        if (!shortcut) {
          var js = 'var $stop_' + num + ' = $B.int_or_bool(' + stop + '),' + h + '        $next' + num + " = " + start + ',' + h + '        $safe' + num + ' = typeof $next' + num + ' == "number" && typeof ' + '$stop_' + num + ' == "number";' + h + '    while(true)';
          var for_node = new $Node();
          new $NodeJSCtx(for_node, js);
          for_node.add($NodeJS('if($safe' + num + ' && $next' + num + '>= $stop_' + num + '){break}'));
          for_node.add($NodeJS('else if(!$safe' + num + ' && $B.ge($next' + num + ', $stop_' + num + ')){break}'));
        ...
        if ($B.last(node.children).C.tree[0].type != "return") {
          var js = '$locals.$line_info = "' + node.line_num + ',' + this.module + '";if($locals.$f_trace !== _b_.None){' + '$B.trace_line()};None;';
        ...
        if (scope.ntype == 'module' && !in_loop) {
          ...
          if (this.has_break) {
            test_range_node.add($NodeJS('var $no_break' + num + ' = $res' + num));
          }
        } else {
      ...
      var it_js = iterable.to_js(),
          ...
          js = 'var ' + iterable_name + ' = ' + it_js + ';' + '$locals["$next' + num + '"]' + ' = $B.$getattr($B.$iter(' + iterable_name + '),"__next__")';
      new $NodeJSCtx(new_node, js);
      new_nodes[pos++] = new_node;

      if (this.has_break) {
        new_nodes[pos++] = $NodeJS(local_ns + '["$no_break' + num + '"] = true;');
      }
      ...
      while_node.add($NodeJS('catch($err){if($B.is_exc($err, [StopIteration]))' + '{break;}else{throw($err)}}'));
      children.forEach(function (child) {
    ...
    this.transform_async = function (node, rank) {
      ...
      new_nodes.push($NodeJS('var ' + type_name + ' = _b_.type.$factory( ' + iterable_name + ')'));
      js = iterable_name + ' = $B.$call($B.$getattr(' + type_name + ', "__aiter__"))(' + iterable_name + ')';
      new_nodes.push($NodeJS(js));
      new_nodes.push($NodeJS('var ' + running_name + ' = true'));
      new_nodes.push($NodeJS('var ' + anext_name + ' = $B.$call($B.$getattr(' + type_name + ', "__anext__"))'));
      ...
      if (target.tree.length == 1) {
        var js = target.to_js() + ' = await ($B.promise(' + anext_name + '(' + iterable_name + ')))';
        ...
        new $RawJSCtx(assign, 'await ($B.promise(' + anext_name + '(' + iterable_name + ')))');
        try_node.add(new_node);
      }

      var catch_node = $NodeJS('catch(err)');
      while_node.add(catch_node);
      var js = 'if(err.__class__ === _b_.StopAsyncIteration)' + '{' + running_name + ' = false; continue}else{throw err}';
    ...
    this.to_js = function () {
      ...
      while (_mod.length > 0) {
        if (_mod.charAt(0) == '.') {
          ...
          if ($package === undefined) {
            return 'throw SystemError.$factory("Parent module \'\' ' + 'not loaded, cannot perform relative import")';
      ...
      if (this.names[0] == '*') {
        scope.blurred = true;
        res[pos++] = '\n' + head + 'for(var attr in module' + '){if(attr.charAt(0) !== "_" && attr.charAt(0) !== "$")' + '{$locals[attr] = module[attr]}};';
      } else {
        this.names.forEach(function (name) {
          module.imports[this.module + '.' + name] = true;
          res[pos++] = '\n' + head + '$locals["' + (this.aliases[name] || name) + '"] = $B.$getattr($B.imported["' + mod_name + '"], "' + name + '");';
        }, this);
      }
  ...
  var $IdCtx = $B.parser.$IdCtx = function (C, value) {
      ...
      switch (ctx.type) {
        ...
        case 'lambda':
          if (ctx.vars === undefined) {
            ctx.vars = [value];
          } else if (ctx.vars.indexOf(value) == -1) {
            ctx.vars.push(value);
          }

          if (this.call_arg && ctx.type == 'lambda') {
            if (ctx.locals === undefined) {
              ctx.locals = [value];
            } else {
              ctx.locals.push(value);
            }
          }
    ...
    if (C.type == 'target_list' || C.type == 'expr' && C.parent.type == 'target_list') {
      ...
      while (!found && node.parent) {
        ...
        for (var i = 0; i < pnode.children.length; i++) {
          ...
          if (child.bindings && child.bindings[this.value]) {
            if ($test) {
              console.log("bound in child", child);
            }
    ...
    this.to_js = function (arg) {
      ...
      this.nonlocal = this.scope.nonlocals && this.scope.nonlocals[val] !== undefined;
      this.unbound = this.unbound || is_local && !this.bound && bound_before && bound_before.indexOf(val) == -1;

      if (!this.bound && this.scope.C && this.scope.ntype == 'class' && this.scope.C.tree[0].name == val) {
        return '$B.$search("' + val + '")';
      }
      ...
      if (this.nonlocal || this.bound) {
        ...
        if ($test) {
          console.log("binding", bscope);
        }

        if (bscope !== undefined) {
          return "$locals_" + bscope.replace(/\./g, "_") + '["' + val + '"]';
        } else if (this.bound) {
          return "$locals_" + innermost.id.replace(/\./g, "_") + '["' + val + '"]';
        }
      }
      ...
      while (1) {
        if (scope.globals !== undefined && scope.globals.has(val)) {
          ...
          if (this.boundBefore(gs)) {
            if ($test) {
              console.log("bound before in gs", gs);
            }

            return global_ns + '["' + val + '"]';
          } else {
            if ($test) {
              console.log("use global search", this);
            }

            if (this.augm_assign) {
              return global_ns + '["' + val + '"]';
            } else {
              return '$B.$global_search("' + val + '", ' + search_ids + ')';
            }
          }
        }

        if ($test) {
          console.log("scope", scope, "innermost", innermost, scope === innermost, "bound_before", bound_before, "found", found.slice());
        }
        ...
        if (scope === innermost) {
          if (bound_before) {
            if (bound_before.indexOf(val) > -1) {
              found.push(scope);
            } else if (scope.C && scope.C.tree[0].type == 'def' && scope.C.tree[0].env.indexOf(val) > -1) {
              found.push(scope);
            }
          } else {
            if (scope.binding[val]) {
              if (this_node.locals[val] === undefined) {
                if (!scope.is_comp && (!scope.parent_block || !scope.parent_block.is_comp)) {
                  found.push(scope);
                }
              } else {
                found.push(scope);
              ...
              if ($test) {
                console.log(val, "found in", scope.id);
              }
            }
          }
        } else {
      ...
      if (found.length > 0) {
        if (found[0].C && found[0] === innermost && val.charAt(0) != '$') {
          var locs = this_node.locals || {},
              nonlocs = innermost.nonlocals;

          if (locs[val] === undefined && (innermost.type != 'def' || innermost.type != 'generator') && innermost.ntype != 'class' && innermost.C.tree[0].args.indexOf(val) == -1 && (nonlocs === undefined || nonlocs[val] === undefined)) {
        ...
        if (found.length > 1 && found[0].C) {
          if (found[0].C.tree[0].type == 'class') {
        ...
        if (scope.C === undefined) {
          if ($test) {
            console.log("module level", scope.id, scope.module);
          }
          ...
          if (scope.id == '__builtins__') {
            ...
            if ($test) {
              console.log("name found at module level");
            }
          ...
          if ($test) {
            console.log("scope is innermost", scope.id);
          }
          ...
          if (scope.ntype == 'generator') {
            ...
            val = '$B.$check_def_free1("' + val + '", "' + scope.id.replace(/\./g, "_") + '")';
          } else {
    ...
    this.to_js = function () {
      ...
      this.tree.forEach(function (item) {
        var mod_name = item.name,
            aliases = item.name == item.alias ? '{}' : '{"' + mod_name + '" : "' + item.alias + '"}',
        ...
        var js = '$B.$import("' + mod_name + '", [],' + aliases + ',' + localns + ', true);';
        res.push(js);
      });
  ...
  var $KwArgCtx = $B.parser.$KwArgCtx = function (C) {
    ...
    if (ctx.kwargs === undefined) {
      ctx.kwargs = [value];
    } else if (ctx.kwargs.indexOf(value) == -1) {
      ctx.kwargs.push(value);
    } else {
      $_SyntaxError(C, ['keyword argument repeated']);
    }
  ...
  var $ListOrTupleCtx = $B.parser.$ListOrTupleCtx = function (C, real) {
    ...
    this.get_src = function () {
      ...
      scope.comments.forEach(function (comment) {
        var start = comment[0],
            len = comment[1];
        src = src.substr(0, start) + ' '.repeat(len + 1) + src.substr(start + len + 1);
      });
      return src;
    };
    ...
    this.bind_ids = function (scope) {
      this.tree.forEach(function (item) {
        ...
        } else if (item.type == 'list_or_tuple' || item.type == "expr" && item.tree[0].type == 'list_or_tuple') {
          if (item.type == "expr") {
            item = item.tree[0];
          }
    ...
    this.packed_indices = function () {
      ...
      for (var i = 0; i < this.tree.length; i++) {
        ...
        if (t.type == "expr") {
          ...
          if (t.type == "packed" || t.type == "call" && t.func.type == "packed") {
            ixs.push(i);
          }
        }
      }
        ...
        case 'dict_or_set_comp':
          ...
          for (var i = 1; i < this.intervals.length; i++) {
            ...
            comments.forEach(function (comment) {
              if (comment[0] > start && comment[0] < end) {
                ...
                txt = txt.substr(0, pos) + ' '.repeat(comment[1]) + txt.substr(pos + comment[1] + 1);
              }
            });
            items.push(txt);
            var lines = txt.split('\n');
          ...
          switch (this.real) {
            case 'list_comp':
              ...
              js = 'function($locals_' + listcomp_name + '){' + js + '})({$list_comp: true})';
    ...
    this.to_js = function () {
      ...
      if (this.tree[0]) {
        ...
        if (this.tree[0].annotation) {
          if (is_not_def) {
            if (this.tree[0].type == "expr" && !this.tree[0].$in_parens && this.tree[0].tree[0].type == "id") {
              var js = "";
              ...
              return js + "_b_.dict.$setitem($locals.__annotations__, '" + this.tree[0].tree[0].value + "', " + this.tree[0].annotation.to_js() + ");";
        ...
        } else if (this.tree[0].type == "assign" && !this.tree[0].tree[0].$in_parens && this.tree[0].tree[0].annotation) {
          ...
          if (left.tree[0] && left.tree[0].type == "id" && is_not_def) {
            this.js += "_b_.dict.$setitem($locals.__annotations__, '" + left.tree[0].value + "', " + left.annotation.to_js() + ");";
          } else {
  ...
  var $NonlocalCtx = $B.parser.$NonlocalCtx = function (C) {
    ...
    this.transform = function (node, rank) {
      ...
      if (pscope.C === undefined) {
        $_SyntaxError(C, ["no binding for nonlocal '" + $B.last(Object.keys(this.names)) + "' found"]);
      } else {
        while (pscope !== undefined && pscope.C !== undefined) {
        ...
        for (var name in this.names) {
          if (!this.names[name][0]) {
            console.log('nonlocal error, C ' + C);
  ...
  var $OpCtx = $B.parser.$OpCtx = function (C, op) {
    ...
    if (C.type == "expr") {
      ...
      } else if (C.tree[0].type == "id") {
        var binding = this.scope.binding[C.tree[0].value];
        ...
        if (this.tree[0].type == 'expr' && this.tree[1].type == 'expr') {
          ...
          switch (t1.type) {
            case 'int':
              switch (t0.type) {
                case 'int':
                  if (Number.isSafeInteger(t0.value) && Number.isSafeInteger(t1.value)) {
                    return js0 + this.op + js1;
                  } else {
                    return '$B.$getattr(' + this.tree[0].to_js() + ',"__' + method + '__")(' + this.tree[1].to_js() + ')';
                  }
                ...
                case 'str':
                  switch (this.op) {
                    ...
                    default:
                      return '$B.$TypeError("unorderable types: ' + " int() " + this.op + ' str()")';
                  }

                case 'id':
                  return 'typeof ' + js0 + ' == "number" ? ' + js0 + this.op + js1 + ' : $B.rich_comp("__' + method + '__",' + this.tree[0].to_js() + ',' + this.tree[1].to_js() + ')';
              }
              ...
              switch (t0.type) {
                ...
                case 'int':
                  switch (this.op) {
                    ...
                    default:
                      return '$B.$TypeError("unorderable types: ' + ' str() ' + this.op + ' int()")';
                  }

                case 'id':
                  return 'typeof ' + js0 + ' == "string" ? ' + js0 + this.op + js1 + ' : $B.rich_comp("__' + method + '__",' + this.tree[0].to_js() + ',' + this.tree[1].to_js() + ')';
              }
            ...
            case 'id':
              if (t0.type == 'id') {
                return 'typeof ' + js0 + '!="object" && typeof ' + js0 + '!="function" && typeof ' + js0 + ' == typeof ' + js1 + ' ? ' + js0 + this.op + js1 + ' : $B.rich_comp("__' + method + '__",' + this.tree[0].to_js() + ',' + this.tree[1].to_js() + ')';
              }
      ...
      switch (this.op) {
        case 'and':
          ...
          if (this.wrap !== undefined) {
            return '(function(){var ' + this.wrap.name + ' = ' + this.wrap.js + ';return $B.$test_expr($B.$test_item(' + op0 + ') && $B.$test_item(' + op1 + '))})()';
          } else {
            return '$B.$test_expr($B.$test_item(' + op0 + ')&&' + '$B.$test_item(' + op1 + '))';
          }
        ...
        case 'unary_inv':
          ...
          if (this.op == 'unary_neg') {
            op = '-';
            method = '__neg__';
          } else if (this.op == 'unary_pos') {
            op = '+';
            method = '__pos__';
          } else {
            op = '~';
        ...
        case 'is':
          return '$B.$is(' + this.tree[0].to_js() + ', ' + this.tree[1].to_js() + ')';

        case 'is_not':
          return this.tree[0].to_js() + '!==' + this.tree[1].to_js();

        case '+':
          return '$B.add(' + this.tree[0].to_js() + ', ' + this.tree[1].to_js() + ')';
        ...
        case '-':
          ...
          var is_simple = function is_simple(elt) {
            if (elt.type == 'expr' && elt.tree[0].type == 'int') {
              return true;
            } else if (elt.type == 'expr' && elt.tree[0].type == 'float') {
              has_float_lit = true;
              return true;
            } else if (elt.type == 'expr' && elt.tree[0].type == 'list_or_tuple' && elt.tree[0].real == 'tuple' && elt.tree[0].tree.length == 1 && elt.tree[0].tree[0].type == 'expr') {
          ...
          var get_type = function get_type(ns, v) {
            var t;
          ...
          if (is_simple(this)) {
            ...
            if (vars.length == 0 && !has_float_lit) {
              ...
              if (t0 == 'float' && t1 == 'float' || this.op == '+' && t0 == 'str' && t1 == 'str') {
                this.result_type = t0;
                return v0.to_js() + this.op + v1.to_js();
              } else if (['int', 'float'].indexOf(t0) > -1 && ['int', 'float'].indexOf(t1) > -1) {
                if (t0 == 'int' && t1 == 'int') {
              ...
              vars.forEach(function (_var) {
                tests.push('typeof ' + _var + '.valueOf() == "number"');
              ...
              if (this.op == '+') {
                res.push(' : (typeof ' + t0 + ' == "string" && typeof ' + t1 + ' == "string") ? ' + t0 + '+' + t1);
              }

              res.push(': $B.rich_op("' + $operators[this.op] + '",' + t0 + ',' + t1 + ')');
              return '(' + res.join('') + ')';
            }
          }

          if (comps[this.op] !== undefined) {
            return '$B.rich_comp("__' + $operators[this.op] + '__",' + e0.to_js() + ',' + e1.to_js() + ')';
          } else {
            return '$B.rich_op("' + $operators[this.op] + '", ' + e0.to_js() + ', ' + e1.to_js() + ')';
          }

        default:
          if (comps[this.op] !== undefined) {
            return '$B.rich_comp("__' + $operators[this.op] + '__",' + this.tree[0].to_js() + ',' + this.tree[1].to_js() + ')';
          } else {
            return '$B.rich_op("' + $operators[this.op] + '", ' + this.tree[0].to_js() + ', ' + this.tree[1].to_js() + ')';
          }
    ...
    this.simple_js = function () {
      ...
      if (op == '+') {
        return '$B.add(' + sjs(this.tree[0]) + ',' + sjs(this.tree[1]) + ')';
      } else if (op == '-') {
        return '$B.sub(' + sjs(this.tree[0]) + ',' + sjs(this.tree[1]) + ')';
      } else if (op == '*') {
        return '$B.mul(' + sjs(this.tree[0]) + ',' + sjs(this.tree[1]) + ')';
      } else if (op == '/') {
        return '$B.div(' + sjs(this.tree[0]) + ',' + sjs(this.tree[1]) + ')';
      } else {
  ...
  var $PackedCtx = $B.parser.$PackedCtx = function (C) {
    ...
    if (C.parent.type == 'list_or_tuple' && C.parent.parent.type == "node") {
      for (var i = 0; i < C.parent.tree.length; i++) {
        var child = C.parent.tree[i];

        if (child.type == 'expr' && child.tree.length > 0 && child.tree[0].type == 'packed') {
  ...
  var $RawJSCtx = $B.parser.$RawJSCtx = function (C, js) {
    this.type = "raw_js";
    C.tree[C.tree.length] = this;
    this.parent = C;
  ...
  var $ReturnCtx = $B.parser.$ReturnCtx = function (C) {
    ...
    if (["def", "generator"].indexOf(this.scope.ntype) == -1) {
      $_SyntaxError(C, ["'return' outside function"]);
    }
  ...
  var $SingleKwCtx = $B.parser.$SingleKwCtx = function (C, token) {
    ...
    if (token == "else") {
      ...
      if (pctx.tree.length > 0) {
        ...
        if (elt.type == 'for' || elt.type == 'asyncfor' || elt.type == 'condition' && elt.token == 'while') {
          elt.has_break = true;
    ...
    this.transform = function (node, rank) {
      if (this.token == 'finally') {
        ...
        if (scope.ntype != 'generator') {
          node.insert(0, $NodeJS('var $exit;' + 'if($B.frames_stack.length<$stack_length){' + '$exit = true;' + '$B.frames_stack.push($top_frame)' + '}'));
          var scope_id = scope.id.replace(/\./g, '_');
          var last_child = node.children[node.children.length - 1];

          if (last_child.C.tree[0].type != "return") {
  ...
  var $StringCtx = $B.parser.$StringCtx = function (C, value) {
    ...
    this.to_js = function () {
        ...
        for (var i = 0; i < parsed_fstring.length; i++) {
          if (parsed_fstring[i].type == 'expression') {
            ...
            for (var j = 0; j < expr_node.children.length; j++) {
              ...
              if (node.C.tree && node.C.tree.length == 1 && node.C.tree[0] == "try") {
            ...
            if (fmt !== undefined) {
              ...
              var res1 = "$B.builtins.str.format('{0:' + " + fmt + " + '}', " + expr1 + ")";
              elts.push(res1);
            } else {
            ...
            var re = new RegExp("'", "g");
            var elt = parsed_fstring[i].replace(re, "\\'").replace(/\n/g, "\\n");
            elts.push("'" + elt + "'");
          }
        }
      ...
      function prepare(value) {
        value = value.replace(/\n/g, '\\n\\\n');
        value = value.replace(/\\U([A-Fa-f0-9]{8})/gm, function (mo) {
      ...
      for (var i = 0; i < this.tree.length; i++) {
        if (this.tree[i].type == "call") {
          var js = '(function(){throw TypeError.$factory("' + "'str'" + ' object is not callable")}())';
          return js;
        } else {
    ...
    this.to_js = function () {
      ...
      if (this.func == 'getitem' && this.value.type == 'id') {
        ...
        if (type == 'list' || type == 'tuple') {
          if (this.tree.length == 1) {
            return '$B.list_key(' + val + ', ' + this.tree[0].to_js() + ')';
          } else if (this.tree.length == 2) {
            return '$B.list_slice(' + val + ', ' + (this.tree[0].to_js() || "null") + ',' + (this.tree[1].to_js() || "null") + ')';
          } else if (this.tree.length == 3) {
            return '$B.list_slice_step(' + val + ', ' + (this.tree[0].to_js() || "null") + ',' + (this.tree[1].to_js() || "null") + ',' + (this.tree[2].to_js() || "null") + ')';
          }
        }
      }
      ...
      if (this.func !== 'delitem' && this.tree.length == 1 && !this.in_sub) {
        var expr = '',
            x = this;
        shortcut = true;
        ...
        var subs = x.value.to_js() + '[' + x.tree[0].to_js() + ']' + '((Array.isArray(' + x.value.to_js() + ') || typeof ' + x.value.to_js() + ' == "string") && ' + subs + ' !== undefined ?' + subs + expr + ' : ';
      }
  ...
  var $TryCtx = $B.parser.$TryCtx = function (C) {
    ...
    this.transform = function (node, rank) {
      ...
      catch_node.add($NodeJS("if($locals.$f_trace !== _b_.None)" + "{$locals.$f_trace = $B.trace_exception()}"));
      catch_node.add($NodeJS(failed_name + ' = true;' + '$B.pmframe = $B.last($B.frames_stack);' + 'if(false){}'));
      var pos = rank + 2,
          has_default = false,
          has_else = false,
          has_finally = false;
      ...
      while (1) {
        if (pos == node.parent.children.length) {
          break;
        }
        ...
        if (ctx.type == 'except') {
          if (has_else) {
            $_SyntaxError(C, "'except' or 'finally' after 'else'");
          }

          if (has_finally) {
            $_SyntaxError(C, "'except' after 'finally'");
          }

          ctx.error_name = error_name;

          if (ctx.tree.length > 0 && ctx.tree[0].alias !== null && ctx.tree[0].alias !== undefined) {
            var alias = ctx.tree[0].alias;
            node.parent.children[pos].insert(0, $NodeJS('$locals["' + alias + '"] = $B.exception(' + error_name + ')'));
          }
          ...
          if (ctx.tree.length == 0) {
            if (has_default) {
              $_SyntaxError(C, 'more than one except: line');
            }
          ...
          if (has_else) {
            $_SyntaxError(C, "more than one 'else'");
          }

          if (has_finally) {
            $_SyntaxError(C, "'else' after 'finally'");
          }
  ...
  var $WithCtx = $B.parser.$WithCtx = function (C) {
    ...
    this.transform = function (node, rank) {
      ...
      if (this.tree[0].alias === null) {
        this.tree[0].alias = '$temp';
      }

      if (this.tree[0].type == 'expr' && this.tree[0].tree[0].type == 'list_or_tuple') {
        if (this.tree[1].type != 'expr' || this.tree[1].tree[0].type != 'list_or_tuple') {
          $_SyntaxError(C);
        }

        if (this.tree[0].tree[0].tree.length != this.tree[1].tree[0].tree.length) {
          $_SyntaxError(C, ['wrong number of alias']);
        }
      ...
      if (this.tree[0].alias) {
        ...
        try_node.add($NodeJS('$locals' + '["' + alias + '"] = ' + this.val_name));
      }
      ...
      var js = this.exc_name + ' = false;' + this.err_name + ' = $B.exception(' + this.err_name + ', true)\n' + ' '.repeat(node.indent + 4) + 'var $b = ' + this.cmexit_name + '(' + this.err_name + '.__class__,' + this.err_name + ',' + '$B.$getattr(' + this.err_name + ', "__traceback__"));';
    ...
    this.transform_async = function (node, rank) {
      ...
      new_nodes.push($NodeJS(js));
      new_nodes.push($NodeJS('    ' + cmtype_name + ' = _b_.type.$factory(' + this.cm_name + '),'));
      new_nodes.push($NodeJS('    ' + this.cmexit_name + ' = $B.$call($B.$getattr(' + cmtype_name + ', "__aexit__")),'));
      new_nodes.push($NodeJS('    ' + cmenter_name + ' = $B.$call($B.$getattr(' + cmtype_name + ', "__aenter__"))' + '(' + this.cm_name + '),'));
      new_nodes.push($NodeJS("    " + this.exc_name + " = false"));
      js = "";

      if (alias) {
        if (alias.tree[0].tree[0].type != "list_or_tuple") {
          var js = alias.tree[0].to_js() + ' = ' + 'await ($B.promise(' + cmenter_name + '))';
          new_nodes.push($NodeJS(js));
        } else {
          var new_node = new $Node(),
              ctx = new $NodeCtx(new_node),
          ...
          var assign = new $AssignCtx(expr);
          new $RawJSCtx(assign, 'await ($B.promise(' + cmenter_name + '))');
      ...
      catch_node.add($NodeJS('var ' + err_name + ' = $B.imported["_sys"].exc_info()'));
      var if_node = $NodeJS('if(! await ($B.promise(' + this.cmexit_name + '(' + this.cm_name + ', ' + err_name + '[0], ' + err_name + '[1], ' + err_name + '[2]))))');
      catch_node.add(if_node);
      if_node.add($NodeJS('$B.$raise()'));
      var else_node = $NodeJS('if(! ' + this.exc_name + ')');
      new_nodes.push(else_node);
      else_node.add($NodeJS('await ($B.promise(' + this.cmexit_name + '(' + this.cm_name + ', _b_.None, _b_.None, _b_.None)))'));
      node.parent.children.splice(rank, 1);
    ...
    this.transform = function (node, rank) {
      ...
      while (parent) {
        if (parent.ctx_manager_num !== undefined) {
          node.parent.insert(rank + 2, $NodeJS("$top_frame[1].$has_yield_in_cm = true"));
          break;
        }
  ...
  var $add_profile = $B.parser.$add_profile = function (node, rank) {
    if (node.type == 'module') {
      ...
      if (elt.type == 'condition' && elt.token == 'elif') {
        flag = false;
      } else if (elt.type == 'except') {
        flag = false;
      } else if (elt.type == 'single_kw') {
        flag = false;
      }
  ...
  var $add_line_num = $B.parser.$add_line_num = function (node, rank, line_info) {
    if (node.type == 'module') {
      ...
      if (flag) {
        _line_info = line_info === undefined ? line_num + ',' + mod_id : line_info;
        var js = ';$locals.$line_info = "' + _line_info + '";if($locals.$f_trace !== _b_.None){$B.trace_line()};' + '_b_.None;';
        var new_node = new $Node();
  ...
  var $previous = $B.parser.$previous = function (C) {
    var previous = C.node.parent.children[C.node.parent.children.length - 2];

    if (!previous || !previous.C) {
      $_SyntaxError(C, 'keyword not following correct keyword');
    }
  ...
  var $get_docstring = $B.parser.$get_docstring = function (node) {
    ...
    if (node.children.length > 0) {
      ...
      if (firstchild.C.tree && firstchild.C.tree.length > 0 && firstchild.C.tree[0].type == 'expr') {
        var expr = firstchild.C.tree[0].tree[0];
  ...
  var $get_line_num = $B.parser.$get_line_num = function (C) {
    var ctx_node = $get_node(C),
  ...
  var $to_js_map = $B.parser.$to_js_map = function (tree_element) {
    if (tree_element.to_js !== undefined) {
      return tree_element.to_js();
    }
    ...
    switch (C.type) {
      case 'abstract_expr':
        ...
        if (!assign) {
          switch (token) {
            ...
            case 'yield':
              ...
              if (assign) {
                console.log("set assign to parent", C);
                C.assign = assign;
              }
        ...
        switch (token) {
          ...
          case 'imaginary':
            return new $ImaginaryCtx(new $ExprCtx(C, 'imaginary', commas), value);
          ...
          case ':':
            if (C.parent.type == "sub" || C.parent.type == "list_or_tuple" && C.parent.parent.type == "sub") {
      ...
      case 'annotation':
        if (token == "eol" && C.tree.length == 1 && C.tree[0].tree.length == 0) {
          $_SyntaxError(C, "empty annotation");
        } else if (token == ':' && C.parent.type != "def") {
          $_SyntaxError(C, "more than one annotation");
      ...
      case 'async':
        if (token == "def") {
          ...
          var ntype = $get_scope(C).ntype;

          if (ntype !== "def" && ntype != "generator") {
            $_SyntaxError(C, ["'async " + token + "' outside async function"]);
          }
      ...
      case 'call_arg':
        switch (token) {
          ...
          case ')':
            if (C.parent.kwargs && $B.last(C.parent.tree).tree[0] && ['kwarg', 'star_arg', 'double_star_arg'].indexOf($B.last(C.parent.tree).tree[0].type) == -1) {
              $_SyntaxError(C, ['non-keyword argument after keyword argument']);
            }
          ...
          case ',':
            if (C.expect == ',') {
              if (C.parent.kwargs && ['kwarg', 'star_arg', 'double_star_arg'].indexOf($B.last(C.parent.tree).tree[0].type) == -1) {
                $_SyntaxError(C, ['non-keyword argument after keyword argument']);
              }
      ...
      case 'condition':
        if (token == ':') {
          if (C.tree[0].type == "abstract_expr" && C.tree[0].tree.length == 0) {
            $_SyntaxError(C, 'token ' + token + ' after ' + C);
          }
      ...
      case 'decorator':
        if (token == 'id' && C.tree.length == 0) {
          return $transition(new $DecoratorExprCtx(C), token, value);
        }
      ...
      case 'decorator_expression':
        if (C.expects === undefined) {
          ...
          $_SyntaxError(C, 'token ' + token + ' after ' + C);
        } else if (C.is_call && token !== "eol") {
          $_SyntaxError(C, 'token ' + token + ' after ' + C);
        } else if (token == "id" && C.expects == "id") {
          C.names.push(value);
          C.expects = ".";
          return C;
        } else if (token == "." && C.expects == ".") {
          C.expects = "id";
          return C;
        } else if (token == "(" && C.expects == ".") {
          if (!C.is_call) {
      ...
      case 'def':
        switch (token) {
          ...
          case '(':
            if (C.name == null) {
              $_SyntaxError(C, "missing name in function definition");
            }
          ...
          case ':':
            if (C.has_args) {
              ...
              $_SyntaxError(C, "missing function parameters");
            }

          case 'eol':
            if (C.has_args) {
              $_SyntaxError(C, "missing colon");
            }
          ...
          if (C.expect == ',') {
            switch (token) {
              ...
              case ',':
                ...
                if (C.real == 'dict' && C.nb_dict_items() % 2) {
                  $_SyntaxError(C, 'token ' + token + ' after ' + C);
                }
              ...
              case 'op':
                switch (value) {
                  ...
                  case '**':
                    ...
                    if (C.real == "dict_or_set") {
                      C.real = value == "*" ? "set" : "dict";
                    } else if (C.real == "set" && value == "**" || C.real == "dict" && value == "*") {
                      $_SyntaxError(C, 'token ' + token + ' after ' + C);
                    }
      ...
      case 'except':
        switch (token) {
          ...
          case ',':
            if (C.parenth !== undefined && C.has_alias === undefined && (C.expect == 'as' || C.expect == ',')) {
              C.expect = 'id';
              return C;
            }
        ...
        switch (token) {
            ...
            if (C.expect == ',') {
              if (C.with_commas) {
                if ($parent_match(C, {
                  ...
                  $_SyntaxError(C, "no implicit tuple for yield from");
                }
          ...
          case 'op':
            ...
            while (1) {
              if (op1.type == 'expr') {
                op1 = op1.parent;
              } else if (op1.type == 'op' && $op_weight[op1.op] >= $op_weight[op] && !(op1.op == '**' && op == '**')) {
                repl = op1;
                op1 = op1.parent;
            ...
            if (repl === null) {
              ...
              if (op === 'and' || op === 'or') {
                while (repl.parent.type == 'not' || repl.parent.type == 'expr' && repl.parent.parent.type == 'not') {
                  repl = repl.parent;
          ...
          case 'augm_assign':
            ...
            while (parent) {
              if (parent.type == "assign" || parent.type == "augm_assign") {
                $_SyntaxError(C, "augmented assignment inside assignment");
            ...
            if (C.expect == ',') {
              return new $AbstractExprCtx(new $AugmentedAssignCtx(C, value), true);
            }
          ...
          case ":":
            if (C.parent.type == "sub" || C.parent.type == "list_or_tuple" && C.parent.parent.type == "sub") {
              ...
              if (C.tree.length == 1) {
                ...
                } else if (child.real == "tuple" && child.expect == "," && child.tree.length == 1) {
          ...
          case '=':
            var has_parent = function has_parent(ctx, type) {
              while (ctx.parent) {
                if (ctx.parent.type == type) {
                  return ctx.parent;
                }
            ...
            if (C.expect == ',') {
              if (C.parent.type == "call_arg") {
                if (C.tree[0].type != "id") {
                  $_SyntaxError(C, ["keyword can't be an expression"]);
                }

                return new $AbstractExprCtx(new $KwArgCtx(C), true);
              } else if (annotation = has_parent(C, "annotation")) {
              ...
              while (C.parent !== undefined) {
                ...
                if (C.type == "condition") {
                  ...
                  $_SyntaxError(C, "assignment inside augmented assignment");
                }
              }
          ...
          case ':=':
            ...
            } else if (ptype == "func_arg_id" && C.parent.tree.length > 0) {
              $_SyntaxError(C, ':= invalid, parent ' + ptype);
            } else if (ptype == "call_arg" && C.parent.parent.type == "call" && C.parent.parent.parent.type == "lambda") {
          ...
          case 'eol':
            if (C.tree.length == 2 && C.tree[0].type == "id" && ["print", "exec"].indexOf(C.tree[0].value) > -1) {
              $_SyntaxError(C, ["Missing parentheses in call " + "to '" + C.tree[0].value + "'."]);
            }

            if (["dict_or_set", "list_or_tuple"].indexOf(C.parent.type) == -1) {
              var t = C.tree[0];
      ...
      case 'for':
        switch (token) {
          ...
          case ':':
            if (C.tree.length < 2 || C.tree[1].tree[0].type == "abstract_expr") {
              $_SyntaxError(C, 'token ' + token + ' after ' + C);
            }
      ...
      case 'from':
        switch (token) {
          case 'id':
            ...
            if (C.expect == 'alias') {
              C.aliases[C.names[C.names.length - 1]] = value;
              C.expect = ',';
              return C;
            }
          ...
          case 'op':
            if (value == '*' && C.expect == 'id' && C.names.length == 0) {
      ...
      case 'func_arg_id':
        switch (token) {
          ...
          case ',':
          case ')':
            if (C.parent.has_default && C.tree.length == 0 && C.parent.has_star_arg === undefined) {
              $pos -= C.name.length;
      ...
      case 'func_args':
        switch (token) {
          case 'id':
            ...
            if (C.expect == 'id') {
              ...
              if (C.names.indexOf(value) > -1) {
                $_SyntaxError(C, ['duplicate argument ' + value + ' in function definition']);
              }
            }
          ...
          case 'op':
            ...
            if (op == '*') {
              if (C.has_star_arg) {
                $_SyntaxError(C, 'duplicate star argument');
              }
              ...
              if (C.has_end_positional) {
                $_SyntaxError(C, ['duplicate / in function parameters']);
              } else if (C.has_star_arg) {
                $_SyntaxError(C, ['/ after * in function parameters']);
              }
      ...
      case 'func_star_arg':
        switch (token) {
          ...
          case ':':
            ...
            if (C.name === undefined) {
              $_SyntaxError(C, 'annotation on an unnamed parameter');
            }
      ...
      case 'id':
        switch (token) {
          case '=':
            if (C.parent.type == 'expr' && C.parent.parent !== undefined && C.parent.parent.type == 'call_arg') {
          ...
          case 'imaginary':
            if (["print", "exec"].indexOf(C.value) > -1) {
              $_SyntaxError(C, ["missing parenthesis in call to '" + C.value + "'"]);
            }
      ...
      case 'import':
        switch (token) {
          case 'id':
            ...
            if (C.expect == 'qual') {
              C.expect = ',';
              C.tree[C.tree.length - 1].name += '.' + value;
              C.tree[C.tree.length - 1].alias += '.' + value;
              return C;
            }
        ...
        if (C.closed) {
          ...
          if (C.expect == ',') {
            switch (C.real) {
              ...
              case 'gen_expr':
                if (token == ')') {
                  while (C.type == "list_or_tuple" && C.real == "tuple" && C.parent.type == "expr" && C.parent.parent.type == "node" && C.tree.length == 1) {
                    var node = C.parent.parent,
                        ix = node.tree.indexOf(C.parent),
                        expr = C.tree[0];
                    expr.parent = node;
                    expr.$in_parens = true;
      ...
      case 'node':
        switch (token) {
          ...
          case 'elif':
            ...
            if (['condition'].indexOf(previous.type) == -1 || previous.token == 'while') {
              $_SyntaxError(C, 'elif after ' + previous.type);
            }
          ...
          case 'else':
            ...
            if (['condition', 'except', 'for'].indexOf(previous.type) == -1) {
              $_SyntaxError(C, 'else after ' + previous.type);
            }
          ...
          case 'finally':
            ...
            if (['try', 'except'].indexOf(previous.type) == -1 && (previous.type != 'single_kw' || previous.token != 'else')) {
              $_SyntaxError(C, 'finally after ' + previous.type);
            }
          ...
          case 'except':
            ...
            if (['try', 'except'].indexOf(previous.type) == -1) {
              $_SyntaxError(C, 'except after ' + previous.type);
            }
      ...
      case 'op':
        ...
        if (C.op.substr(0, 5) == 'unary' && token != 'eol') {
        ...
        switch (token) {
          ...
          default:
            if (C.tree[C.tree.length - 1].type == 'abstract_expr') {
              $_SyntaxError(C, 'token ' + token + ' after ' + C);
            }
      ...
      case 'star_arg':
        switch (token) {
          ...
          case ')':
            if (C.tree.length == 0) {
              $_SyntaxError(C, "unnamed star argument");
            }
      ...
      case 'target_list':
        switch (token) {
          case 'id':
            if (C.expect == 'id') {
              C.expect = ',';
              return new $IdCtx(new $ExprCtx(C, 'target', false), value);
            }
      ...
      case 'with':
        switch (token) {
          ...
          case ',':
            if (C.parenth !== undefined && C.has_alias === undefined && (C.expect == ',' || C.expect == 'as')) {
              C.expect = 'id';
              return C;
            } else if (C.expect == 'as') {
              C.expect = 'id';
  ...
  var $tokenize = $B.parser.$tokenize = function (root, src) {
    ...
    var _loop2 = function _loop2() {
      ...
      if (indent === null) {
        ...
        } else if (indent <= current.indent && C && C.tree[0] && $indented.indexOf(C.tree[0].type) > -1 && C.tree.length < 2) {
          $pos = pos;
          ...
          if (escaped) {
            ...
            if (raw) {
              ...
              if (src.charAt(end + 1) == '\n') {
                ...
                if (search === null) {
                  $_SyntaxError(C, "(unicode error) " + "malformed \\N character escape", pos);
                }
                ...
                if ($B.unicodedb !== undefined) {
                  re = new RegExp("^([0-9A-F]+);" + description + ";.*$", "m");
                  search = re.exec($B.unicodedb);

                  if (search === null) {
                    $_SyntaxError(C, "(unicode error) " + "unknown Unicode character name", pos);
                  }
                ...
                if (end < src.length - 1 && is_escaped[src.charAt(end + 1)] === undefined) {
                  zone += '\\';
                }
            ...
            $pos = end;
            $_SyntaxError(C, ["EOL while scanning string literal"]);
            ...
            if (_type == "triple_string" && src.substr(end, 3) != car + car + car) {
              ...
              found = true;
              $pos = pos;
              $string = zone.substr(1);
              string = '';

              for (i = 0; i < $string.length; i++) {
                $car = $string.charAt(i);

                if ($car == car && (raw || i == 0 || $string.charAt(i - 1) != '\\')) {
                  string += '\\';
                }
              ...
              if (fstring) {
                try {
                  ...
                  elts = $B.parse_fstring(string_no_bs);
                } catch (err) {
                  $_SyntaxError(C, [err.toString()]);
                }
              }
      ...
      if (name == "" && car != '$') {
        if ($B.unicode_tables.XID_Start[car.charCodeAt(0)]) {
          ...
          while (pos < src.length && $B.unicode_tables.XID_Continue[src.charCodeAt(pos)]) {
            name += src.charAt(pos);
            pos++;
          }
        }
        ...
        if (name) {
          if (kwdict.indexOf(name) > -1) {
            ...
            C = $transition(C, name);
          } else if (typeof $operators[name] == 'string' && ['is_not', 'not_in'].indexOf(name) == -1) {
            if (name == 'is') {
              re = /^\s+not\s+/;
              res = re.exec(src.substr(pos));

              if (res !== null) {
                pos += res[0].length;
                $pos = pos - name.length;
                C = $transition(C, 'op', 'is_not');
              } else {
                $pos = pos - name.length;
          ...
          } else if ((src.charAt(pos) == '"' || src.charAt(pos) == "'") && ['r', 'b', 'u', 'rb', 'br', 'f', 'fr', 'rf'].indexOf(name.toLowerCase()) !== -1) {
      ...
      switch (car) {
        ...
        case '.':
          if (pos < src.length - 1 && /^\d$/.test(src.charAt(pos + 1))) {
            ...
            while (j < src.length && src.charAt(j).search(/\d|e|E|_/) > -1) {
              j++;
            }

            if (src.charAt(j) == "j") {
              C = $transition(C, 'imaginary', '0' + rmu(src.substr(pos, j - pos)));
              j++;
            } else {
              C = $transition(C, 'float', '0' + rmu(src.substr(pos, j - pos)));
            }
        ...
        case '0':
          res = hex_pattern.exec(src.substr(pos));
          ...
          res = octal_pattern.exec(src.substr(pos));

          if (res) {
            C = $transition(C, 'int', [8, rmuf(res[1])]);
            pos += res[0].length;
            break;
          }

          res = binary_pattern.exec(src.substr(pos));

          if (res) {
            C = $transition(C, 'int', [2, rmuf(res[1])]);
            pos += res[0].length;
            break;
          }
        ...
        case '8':
        case '9':
          res = float_pattern1.exec(src.substr(pos));
          ...
          if (res) {
            ...
            if ($B.last(res) !== undefined) {
              C = $transition(C, 'imaginary', rmuf(res[0].substr(0, res[0].length - 1)));
            } else {
              C = $transition(C, 'float', rmuf(res[0]));
            }
          } else {
            ...
            if (res) {
              ...
              if (res[2] !== undefined) {
                C = $transition(C, 'imaginary', rmu(res[1]));
              } else {
                C = $transition(C, 'int', [10, rmu(res[0])]);
              }
            }
        ...
        case '\n':
          ...
          if (br_stack.length > 0) {
            ...
            if (current.C.tree.length > 0 || current.C.async) {
              $pos = pos;
              C = $transition(C, 'eol');
              indent = null;
              new_node = new $Node();
        ...
        case '!':
          ...
          for (op_sign in $operators) {
            if (op_sign == src.substr(pos, op_sign.length) && op_sign.length > op_match.length) {
              op_match = op_sign;
            }
          }
          ...
          if (op_match.length > 0) {
            ...
            $_SyntaxError(C, 'invalid character: ' + car);
          }
    ...
    _loop: while (pos < src.length) {
      ...
      var offset;
      var end;
      var raw, bytes, fstring, sm_length, end;
      var escaped, zone, found;
      var end_lit, re, search;
      var end_lit;
      var description;
      var xhr;
      var re;
    ...
    if (br_stack.length != 0) {
      ...
      $_SyntaxError(br_err[0], ["unexpected EOF while parsing (" + fname + ", line " + (lines.length - 1) + ")"]);
    }
    ...
    if (C !== null) {
      ...
      } else if (C.tree[0] && $indented.indexOf(C.tree[0].type) > -1) {
        $pos = pos - 1;
        $_SyntaxError(C, 'expected an indented block', pos);
      } else {
        var parent = current.parent;

        if (parent.C && parent.C.tree && parent.C.tree[0] && parent.C.tree[0].type == "try") {
          $pos = pos - 1;
          $_SyntaxError(C, ["unexpected EOF while parsing"]);
        }
      }
  ...
  $B.py2js = function (src, module, locals_id, parent_scope, line_num) {
    ...
    var enter_frame_pos = offset,
        js = 'var $top_frame = ["' + locals_id.replace(/\./g, '_') + '", ' + local_ns + ', "' + module.replace(/\./g, '_') + '", ' + global_ns + ']\n$locals.$f_trace = $B.enter_frame($top_frame)\n' + 'var $stack_length = $B.frames_stack.length;';
  ...
  var brython = $B.parser.brython = function (options) {
    ...
    if (options.python_paths) {
      options.python_paths.forEach(function (path) {
        ...
        if (path.slice(-7).toLowerCase() == '.vfs.js' && (prefetch === undefined || prefetch === true)) {
  ...
  $B.run_script = function (src, name, run_loop) {
    ...
    try {
      var root = $B.py2js(src, name, name),
        ...
        __name__: name,
        $src: src,
        __file__: $B.script_path + ($B.script_path.endsWith("/") ? "" : "/") + name
      };
  ...
  var _run_scripts = $B.parser._run_scripts = function (options) {
        ...
        if (script.type == "text/python" || script.type == "text/python3") {
          if (script.className == "webworker") {
            if (script.id === undefined) {
              throw _b_.AttributeError.$factory("webworker script has no attribute 'id'");
            }
    ...
    if (options.ipy_id !== undefined) {
      ...
      if ($elts.length > 0) {
        if (options.indexedDB && $B.has_indexedDB && $B.hasOwnProperty("VFS")) {
          $B.tasks.push([$B.idb_open]);
        }
      }
      ...
      for (var i = 0; i < $elts.length; i++) {
        ...
        if (elt.id) {
          if (defined_ids[elt.id]) {
            throw Error("Brython error : Found 2 scripts with the " + "same id '" + elt.id + "'");
          } else {
...
(function ($B) {
    ...
    if (res === undefined || res.timestamp != $B.timestamp) {
      ...
      if (res.imports.length > 0) {
        ...
        for (var i = 0; i < subimports.length; i++) {
          ...
          if (!$B.imported.hasOwnProperty(subimport) && !$B.precompiled.hasOwnProperty(subimport)) {
    ...
    idb_cx.onsuccess = function () {
      ...
      if (!db.objectStoreNames.contains("modules")) {
        ...
        idb_cx.onupgradeneeded = function () {
          ...
          var db = $B.idb_cx.result,
              store = db.createObjectStore("modules", {
            "keyPath": "name"
          });
        ...
        store.openCursor().onsuccess = function (evt) {
          ...
          if (cursor) {
            ...
            if (record.timestamp == $B.timestamp) {
              if (!$B.VFS || !$B.VFS[record.name] || $B.VFS[record.name].timestamp == record.source_ts) {
  ...
  $B.ajax_load_script = function (script) {
    ...
    if ($B.files && $B.files.hasOwnProperty(name)) {
      $B.tasks.splice(0, 0, [$B.run_script, $B.files[name], name, true]);
      ...
      throw _b_.IOError.$factory("can't load external script at " + script.url + " (Ajax calls not supported with protocol file:///)");
    }
  ...
  function add_jsmodule(module, source) {
    source += "\nvar $locals_" + module.replace(/\./g, "_") + " = $module";
    $B.precompiled[module] = source;
  }

  var inImported = $B.inImported = function (module) {
    if ($B.imported.hasOwnProperty(module)) {} else if (__BRYTHON__.VFS && __BRYTHON__.VFS.hasOwnProperty(module)) {
      ...
      if ($B.idb_cx) {
        ...
        while ($B.outdated.length > 0) {
          ...
          req.onsuccess = function (event) {
            ...
            document.dispatchEvent(new CustomEvent('precompile', {
              detail: 'remove outdated ' + module + ' from cache'
            }));
          };
        }
    ...
    for (var i = start; i < imports.length; i++) {
      ...
      if (mod_obj[0] == ".py") {
        ...
        subimports.forEach(function (subimport) {
          if (!$B.imported.hasOwnProperty(subimport) && imports.indexOf(subimport) == -1) {
            if ($B.VFS.hasOwnProperty(subimport)) {
...
__BRYTHON__.builtins.object = function ($B) {
  ...
  object.__delattr__ = function (self, attr) {
    ...
    if (self.__dict__ && self.__dict__.$string_dict && self.__dict__.$string_dict[attr] !== undefined) {
  ...
  object.__getattribute__ = function (obj, attr) {
    ...
    if (res === undefined && obj.__dict__ && obj.__dict__.$string_dict.hasOwnProperty(attr)) {
    ...
    if (res !== undefined) {
      ...
      if (res.__class__ === $B.method) {
        if ($test) {
          console.log("res is method");
        }

        if (res.__get__ === undefined) {
          console.log("bizarre", obj, attr, res);
        }
  ...
  object.__reduce__ = function (self) {
    ...
    _reconstructor.$infos = {
      __qualname__: "_reconstructor"
    };
    ...
    for (var attr in self.__dict__.$string_dict) {
      _b_.dict.$setitem(d.$string_dict, attr, self.__dict__.$string_dict[attr][0]);
    }
  ...
  object.__repr__ = function (self) {
    ...
    if (module !== undefined && !module.startsWith("$") && module !== "builtins") {
      return "<" + self.__class__.$infos.__module__ + "." + $B.class_name(self) + " object>";
    } else {
...
(function ($B) {
  ...
  $B.$class_constructor = function (class_name, class_obj, bases, parents_names, kwargs) {
    ...
    for (var i = 0; i < bases.length; i++) {
      if (bases[i] === undefined || bases[i].__mro__ === undefined && bases[i].__class__ !== $B.JSObject) {
        var mro_entries = $B.$getattr(bases[i], "__mro_entries__", _b_.None);
        ...
        if (mro_entries !== _b_.None) {
          ...
          (_bases = bases).splice.apply(_bases, [i, 1].concat(_toConsumableArray(entries)));
    ...
    if (metaclass === undefined) {
      if (bases && bases.length > 0 && bases[0].__class__ !== $B.JSObject) {
        ...
        for (var i = 1; i < bases.length; i++) {
          ...
          if (mc === metaclass) {} else if (mc.__bases__ && mc.__bases__.indexOf(metaclass) > -1) {
            metaclass = mc;
          } else if (metaclass.__bases__ && metaclass.__bases__.indexOf(mc) == -1) {
            throw _b_.TypeError.$factory("metaclass conflict: the " + "metaclass of a derived class must be a (non-" + "strict) subclass of the metaclasses of all its bases");
          }
        }
      } else {
        metaclass = _b_.type;
    ...
    var prepare = $B.$getattr(metaclass, "__prepare__", _b_.None),
        cl_dict = prepare(class_name, bases);

    if (cl_dict.__class__ !== _b_.dict) {
      set_class_item = $B.$getattr(cl_dict, "__setitem__");
    } else {
      set_class_item = function set_class_item(attr, value) {
    ...
    for (var i = 0; i < mro.length - 1; i++) {
      for (var attr in mro[i]) {
        if (attr == "__setattr__") {
          ...
          if (mro[i][attr].__get__ || mro[i][attr].__class__ && mro[i][attr].__class__.__get__) {
            cl_dict.$has_setattr = true;
            break;
          }
        }
    ...
    for (var i = 0; i < bases.length; i++) {
      ...
      if (i == 0) {
        var init_subclass = _b_.type.__getattribute__(bases[i], "__init_subclass__");

        if (init_subclass.$infos.__func__ !== undefined) {
          init_subclass.$infos.__func__(kls, {
            $nat: "kw",
            kw: extra_kwargs
          });
        } else {
  ...
  type.__getattribute__ = function (klass, attr) {
    switch (attr) {
        ...
        for (var i = 0, len = mro.length; i < len; i++) {
          if (mro[i].__dict__) {
            ...
            if (ann) {
              if (res === undefined) {
                res = ann;
              } else if (res.__class__ === _b_.dict && ann.__class__ === _b_.dict) {
      ...
      case "__setattr__":
        if (klass["__setattr__"] !== undefined) {
          ...
          var func = function func(obj, key, value) {
            obj[key] = value;
          };
        }
    ...
    if (res === undefined && klass.__slots__ && klass.__slots__.indexOf(attr) > -1) {
    ...
    if (res === undefined) {
      ...
      if (res === undefined) {
        ...
        if (res !== undefined) {
          if ($test) {
            console.log("found in meta", res, _typeof(res));
          }
  ...
  type.__name__ = {
    __get__: function __get__(self) {
      return self.$infos.__name__;
    },
    __set__: function __set__(self, value) {
  ...
  type.__repr__ = type.__str__ = function (kls) {
    ...
    if (kls.$infos.__module__ && kls.$infos.__module__ != "builtins" && !kls.$infos.__module__.startsWith("$")) {
      ...
      if (bases[i] === undefined || bases[i].__mro__ === undefined) {
        if (bases[i].__class__ === $B.JSObject) {
          ...
          bases[i] = {
            ...
            __init__: function __init__(instance) {
      ...
      if (klass.hasOwnProperty("__new__")) {
        if (klass.hasOwnProperty("__init__")) {
          factory = function factory() {
            var obj = klass.__new__.bind(null, klass).apply(null, arguments);
        ...
        factory = function factory() {
          if (arguments.length > 0) {
            if (arguments.length == 1 && arguments[0].$nat && Object.keys(arguments[0].kw).length == 0) {} else {
              throw _b_.TypeError.$factory("object() takes no parameters");
            }
          }
  ...
  var member_descriptor = $B.make_class("member_descriptor", function (attr, cls) {
    return {
  ...
  member_descriptor.__str__ = member_descriptor.__repr__ = function (self) {
    return "<member '" + self.attr + "' of '" + self.cls.$infos.__name__ + "' objects>";
  };
  ...
  method.__setattr__ = function (self, key, value) {
    if (key == "__class__") {
      throw _b_.TypeError.$factory("__class__ assignment only supported " + "for heap types or ModuleType subclasses");
    }

    throw _b_.AttributeError.$factory("'method' object has no attribute '" + key + "'");
  };
...
(function ($B) {
  var _b_ = $B.builtins,
      _window = self,
      isWebWorker = 'undefined' !== typeof WorkerGlobalScope && "function" === typeof importScripts && navigator instanceof WorkerNavigator;
  ...
  $B.args = function ($fname, argcount, slots, var_names, args, $dobj, extra_pos_args, extra_kw_args) {
    ...
    if (nb_pos > 0 && $args[nb_pos - 1] && $args[nb_pos - 1].$nat) {
      ...
      if (Object.keys($args[nb_pos].kw).length > 0) {
        ...
        if (Array.isArray(kw_args)) {
          kw_args = $B.extend.apply($B, [$fname].concat(_toConsumableArray(kw_args)));
        }
      }
    }
    ...
    if (nb_pos > argcount) {
      if (extra_pos_args === null || extra_pos_args == "*") {
        msg = $fname + "() takes " + argcount + " positional argument" + (argcount > 1 ? "s" : "") + " but more were given";
    ...
    if (filled == argcount && argcount === var_names.length && !has_kw_args) {
      if (extra_kw_args) {
  ...
  $B.wrong_nb_args = function (name, received, expected, positional) {
    if (received < expected) {
      ...
      throw _b_.TypeError.$factory(name + "() missing " + missing + " positional argument" + (missing > 1 ? "s" : "") + ": " + positional.slice(received));
    } else {
      throw _b_.TypeError.$factory(name + "() takes " + expected + " positional argument" + (expected > 1 ? "s" : "") + " but more were given");
    }
  };
  ...
  $B.$dict_comp = function (module_name, parent_scope, items, line_num) {
    ...
    var dictcomp_name = "dc" + ix,
      ...
      line_info: line_info
        ...
        js = root.to_js();
    js += '\nreturn $locals["' + res + '"]\n';
    js = "(function($locals_" + dictcomp_name + "){" + js + "})({$dict_comp: true})";
  ...
  $B.$search = function (name, global_ns) {
    ...
    if (frame[1][name] !== undefined) {
      ...
      if (frame[0] == frame[2] || frame[1].$type == "class" || frame[1].$exec_locals) {
        throw _b_.NameError.$factory("name '" + name + "' is not defined");
      } else {
        throw _b_.UnboundLocalError.$factory("local variable '" + name + "' referenced before assignment");
      }
    }
  };
  ...
  $B.$global_search = function (name, search_ids) {
    ...
    for (var i = 0; i < $B.frames_stack.length; i++) {
      ...
      if (search_ids.indexOf(frame[0]) > -1 && frame[1][name] !== undefined) {
        return frame[1][name];
      }

      if (search_ids.indexOf(frame[2]) > -1 && frame[3][name] !== undefined) {
        return frame[3][name];
      }
    }
  ...
  $B.$local_search = function (name) {
    ...
    if (frame[1][name] !== undefined) {
      ...
      throw _b_.UnboundLocalError.$factory("local variable '" + $B.from_alias(name) + "' referenced before assignment");
    }
  };
  ...
  $B.$check_def_free = function (name, value) {
    ...
    throw _b_.NameError.$factory("free variable '" + $B.from_alias(name) + "' referenced before assignment in enclosing scope");
  };
  ...
  $B.$JS2Py = function (src) {
    ...
    if (_typeof(src) == "object") {
      if ($B.$isNode(src)) {
        return $B.DOMNode.$factory(src);
      }

      if ($B.$isEvent(src)) {
        return $B.$DOMEvent(src);
      }
  ...
  $B.list_slice_step = function (obj, start, stop, step) {
    if (step === null || step == 1) {
      return $B.list_slice(obj, start, stop);
    }
    ...
    if (start === null) {
      ...
      if (start < 0) {
        start = Math.min(0, start + obj.length);
      }
    }
  ...
  $B.$getitem = function (obj, item) {
    ...
    if ((is_list || typeof obj == "string") && typeof item == "number") {
      item = item >= 0 ? item : obj.length + item;
    ...
    throw _b_.TypeError.$factory("'" + $B.class_name(obj) + "' object is not subscriptable");
  };
  ...
  $B.set_list_slice_step = function (obj, start, stop, step, value) {
    ...
    if (nb != repl.length) {
      throw _b_.ValueError.$factory("attempt to assign sequence of size " + repl.length + " to extended slice of size " + nb);
    }
  ...
  $B.$setitem = function (obj, item, value) {
    if (Array.isArray(obj) && obj.__class__ === undefined && typeof item == "number" && !_b_.isinstance(obj, _b_.tuple)) {
      if (item < 0) {
        item += obj.length;
      }
  ...
  $B.augm_item_add = function (obj, item, incr) {
    ...
    try {
      ...
      ga(obj, "__setitem__")(item, ga(ga(obj, "__getitem__")(item), "__add__")(incr));
      return;
    }
  ...
  $B.$is_member = function (item, _set) {
    ...
    try {
      method = $B.$getattr(_set.__class__ || $B.get_class(_set), "__contains__");
    } catch (err) {}
    ...
    if (_iter) {
      while (1) {
        try {
          ...
          if ($B.rich_comp("__eq__", elt, item)) {
            return true;
          }
        } catch (err) {
          return false;
    ...
    try {
      ...
      throw _b_.TypeError.$factory("'" + $B.class_name(_set) + "' object is not iterable");
    }
  ...
  $B.$call = function (callable) {
    if (callable.__class__ === $B.method) {
      ...
      if (typeof callable.js == "function") {
        ...
        throw _b_.TypeError.$factory("'" + $B.class_name(callable) + "' object is not callable");
      }
    }
  ...
  $B.make_iterator_class = function (name) {
    var klass = {
      __class__: _b_.type,
      __mro__: [_b_.object],
      $factory: function $factory(items) {
        return {
          __class__: klass,
          __dict__: _b_.dict.$factory(),
          counter: -1,
      ...
      __next__: function __next__(self) {
        if (typeof self.len_func == "function" && self.len_func() != self.len) {
      ...
      __reduce_ex__: function __reduce_ex__(self, protocol) {
  ...
  function $err(op, klass, other) {
    var msg = "unsupported operand type(s) for " + op + ": '" + klass.$infos.__name__ + "' and '" + $B.class_name(other) + "'";
    throw _b_.TypeError.$factory(msg);
  }
      ...
      if (klass["__" + ropnames[j] + "__"] === undefined) {
        klass["__" + ropnames[j] + "__"] = function (name, sign) {
          return function (self, other) {
            try {
              return $B.$getattr(other, "__r" + name + "__")(self);
            } catch (err) {
              $err(sign, klass, other);
            }
          };
  ...
  $B.$GetInt = function (value) {
    ...
    throw _b_.TypeError.$factory("'" + $B.class_name(value) + "' object cannot be interpreted as an integer");
  };
    ...
    for (var i = 0; i < methods.length; i++) {
      ...
      if (method !== missing) {
        ...
        if (!_b_.isinstance(res, expected_class[methods[i]])) {
          ...
          throw _b_.TypeError.$factory(methods[i] + "returned non-" + expected_class[methods[i]].$infos.__name__ + "(type " + $B.get_class(res) + ")");
        }
  ...
  $B.PyNumber_Index = function (item) {
    switch (_typeof(item)) {
      ...
      case "object":
        ...
        if (method !== _b_.None) {
          method = typeof method == "function" ? method : $B.$getattr(method, "__call__");
          return $B.int_or_bool(method);
        }

      default:
        throw _b_.TypeError.$factory("'" + $B.class_name(item) + "' object cannot be interpreted as an integer");
    }
  };
    ...
    switch (_typeof(v)) {
      ...
      case "object":
        if (v.__class__ === $B.long_int) {
          ...
          throw _b_.TypeError.$factory("'" + $B.class_name(v) + "' object cannot be interpreted as an integer");
        }
  ...
  $B.trace_exception = function () {
    var top_frame = $B.last($B.frames_stack),
        ...
        frame_obj = $B._frame.$factory($B.frames_stack, $B.frames_stack.length - 1);
  ...
  $B.set_line = function (line_info) {
    ...
    if (trace_func !== _b_.None) {
      var frame_obj = $B._frame.$factory($B.frames_stack, $B.frames_stack.length - 1);
  ...
  $B.add = function (x, y) {
    if (typeof x.valueOf() == "number" && typeof y.valueOf() == "number") {
      if (typeof x == "number" && typeof y == "number") {
        ...
        return $B.long_int.__add__($B.long_int.$factory(x), $B.long_int.$factory(y));
      } else {
    ...
    try {
      ...
      if (err.__class__ === _b_.AttributeError) {
        throw _b_.TypeError.$factory("unsupported operand type(s) for " + "+: '" + $B.class_name(x) + "' and '" + $B.class_name(y) + "'");
      }
  ...
  $B.div = function (x, y) {
    ...
    if (x > min_int && x < max_int && y > min_int && y < max_int && z > min_int && z < max_int) {
      return z;
    } else {
  ...
  $B.mul = function (x, y) {
    var z = typeof x != "number" || typeof y != "number" ? new Number(x * y) : x * y;

    if (x > min_int && x < max_int && y > min_int && y < max_int && z > min_int && z < max_int) {
      return z;
    } else if ((typeof x == "number" || x.__class__ === $B.long_int) && (typeof y == "number" || y.__class__ === $B.long_int)) {
  ...
  $B.rich_comp = function (op, x, y) {
    ...
    if (typeof x1 == "number" && typeof y1 == "number" && x.__class__ === undefined && y.__class__ === undefined) {
      switch (op) {
        case "__eq__":
          return x1 == y1;
    ...
    if (x.$is_class || x.$factory) {
      if (op == "__eq__") {
        ...
        throw _b_.TypeError.$factory("'" + method2comp[op] + "' not supported between instances of '" + $B.class_name(x) + "' and '" + $B.class_name(y) + "'");
      }
    }
  ...
  $B.rich_op = function (op, x, y) {
    ...
    if (x_class === y_class) {
      try {
        ...
        if (err.__class__ === _b_.AttributeError) {
          ...
          throw _b_.TypeError.$factory("unsupported operand type(s) " + "for " + opname2opsign[op] + ": '" + kl_name + "' and '" + kl_name + "'");
        }
    ...
    try {
      ...
      throw _b_.TypeError.$factory("'" + (opname2opsign[op] || op) + "' not supported between instances of '" + $B.class_name(x) + "' and '" + $B.class_name(y) + "'");
    }
...
(function ($B) {
  ...
  var check_nb_args = $B.check_nb_args = function (name, expected, args) {
    ...
    if (len != expected) {
      if (expected == 0) {
        throw _b_.TypeError.$factory(name + "() takes no argument" + " (" + len + " given)");
      } else {
        throw _b_.TypeError.$factory(name + "() takes exactly " + expected + " argument" + (expected < 2 ? '' : 's') + " (" + len + " given)");
      }
    }
  };
  ...
  var check_no_kw = $B.check_no_kw = function (name, x, y) {
    ...
    if (x.$nat && x.kw && x.kw[0] && x.kw[0].length > 0 || y !== undefined && y.$nat) {
  ...
  var NoneType = {
    ...
    __bool__: function __bool__(self) {
      return False;
    },
    __class__: _b_.type,
    __hash__: function __hash__(self) {
      return 0;
    },
    __mro__: [object],
  ...
  function abs(obj) {
    ...
    try {
      ...
      if (err.__class__ === _b_.AttributeError) {
        throw _b_.TypeError.$factory("Bad operand type for abs(): '" + $B.class_name(obj) + "'");
      }
  ...
  function bin_hex_oct(base, obj) {
    if (isinstance(obj, _b_.int)) {
      ...
      try {
        ...
        if (err.__class__ === _b_.AttributeError) {
          throw _b_.TypeError.$factory("'" + $B.class_name(obj) + "' object cannot be interpreted as an integer");
        }
  ...
  var classmethod = $B.make_class("classmethod", function (func) {
    ...
    f.__get__ = function (obj, cls) {
      var method = function method() {
        return f.apply(void 0, [cls].concat(Array.prototype.slice.call(arguments)));
      };
  ...
  function compile() {
    var $ = $B.args('compile', 6, {
      ...
      dont_inherit: null,
      optimize: null
    }, ['source', 'filename', 'mode', 'flags', 'dont_inherit', 'optimize'], arguments, {
      flags: 0,
      dont_inherit: false,
      optimize: -1
    }, null, null);
  ...
  function delattr(obj, attr) {
    ...
    if (typeof attr != 'string') {
      throw _b_.TypeError.$factory("attribute name must be string, not '" + $B.class_name(attr) + "'");
    }
  ...
  function $$eval(src, _globals, _locals) {
    ...
    if (_globals === _b_.None) {
      ...
      if (_globals.__class__ != _b_.dict) {
        throw _b_.TypeError.$factory("exec() globals must be a dict, not " + $B.class_name(_globals));
      }
    ...
    if (_globals === _b_.None && _locals === _b_.None && current_frame[0] == current_frame[2]) {} else {
    ...
    try {
      ...
      if (mode != "eval") {
        ...
        if (["node_js"].indexOf(last.C.type) == -1) {
          last.to_js = function () {
            while (js.endsWith("\n")) {
        ...
        if (_globals === _b_.None) {
          var res = new Function("$locals_" + globals_id, "$locals_" + locals_id, js)(globals_obj, locals_obj);
        } else {
      ...
      if ($.src.filename == "<console>" && $.src.mode == "single" && res !== undefined && res !== _b_.None) {
        _b_.print(res);
      }
  ...
  function format(value, format_spec) {
    ...
    try {
      ...
      if (err.__class__ === _b_.AttributeError) {
        throw _b_.NotImplementedError("__format__ is not implemented " + "for object '" + _b_.str.$factory(value) + "'");
      }
  ...
  $B.$getattr = function (obj, attr, _default) {
    ...
    if (klass !== undefined && klass.__bases__ && (klass.__bases__.length == 0 || klass.__bases__.length == 1 && klass.__bases__[0] === _b_.object)) {
      if (obj.hasOwnProperty(attr)) {
        return obj[attr];
      } else if (obj.__dict__ && obj.__dict__.$string_dict.hasOwnProperty(attr) && !(klass.hasOwnProperty(attr) && klass[attr].__get__)) {
        return obj.__dict__.$string_dict[attr][0];
    ...
    switch (attr) {
      ...
      case '__dict__':
        if (is_class) {
          return $B.mappingproxy.$factory(obj);
        } else {
          if (obj.hasOwnProperty(attr)) {
            return obj[attr];
          } else if (obj.$infos) {
    ...
    if (!is_class && klass.$native) {
      ...
      if (klass[attr] === undefined) {
        ...
        if (object_attr !== undefined) {
          ...
          if (attrs && (object_attr = attrs.$string_dict[attr]) !== undefined) {
  ...
  function hash(obj) {
    ...
    if (obj.$is_class || obj.__class__ === _b_.type || obj.__class__ === $B.Function) {
    ...
    if (hash_method == _b_.None) {
      throw _b_.TypeError.$factory("unhashable type: '" + $B.class_name(obj) + "'", 'hash');
    }
  ...
  function id(obj) {
    ...
    if (isinstance(obj, [_b_.str, _b_.int, _b_.float]) && !isinstance(obj, $B.long_int)) {
  ...
  function isinstance(obj, cls) {
    ...
    if (!cls.__class__ || !(cls.$factory !== undefined || cls.$is_class !== undefined)) {
      throw _b_.TypeError.$factory("isinstance() arg 2 must be a type " + "or tuple of types");
    }
  ...
  function issubclass(klass, classinfo) {
    ...
    if (classinfo === _b_.type || classinfo.__bases__ && classinfo.__bases__.indexOf(_b_.type) > -1) {
    ...
    if (sentinel === undefined) {
      ...
      try {
        ...
        if (err.__class__ === _b_.AttributeError) {
          try {
            ...
            throw _b_.TypeError.$factory("'" + $B.class_name(obj) + "' object is not iterable");
          }
        }
      ...
      try {
        ...
        if (isinstance(err, _b_.AttributeError)) {
          throw _b_.TypeError.$factory("iter() returned non-iterator of type '" + $B.class_name(res) + "'");
        }
      }
  ...
  function len(obj) {
    ...
    try {
      ...
      throw _b_.TypeError.$factory("object of type '" + $B.class_name(obj) + "' has no len()");
    }
  ...
  var memoryview = $B.make_class('memoryview', function (obj) {
    ...
    if ($B.get_class(obj).$buffer_protocol) {
      ...
      throw _b_.TypeError.$factory("memoryview: a bytes-like object " + "is required, not '" + $B.class_name(obj) + "'");
    }
  });
  ...
  memoryview.cast = function (self, format) {
    switch (format) {
      ...
      case "I":
        ...
        if (objlen % 4 != 0) {
          throw _b_.TypeError.$factory("memoryview: length is not " + "a multiple of itemsize");
        }
  ...
  function next(obj) {
    ...
    if (ga !== undefined) {
      try {
        ...
        if (err.__class__ === _b_.StopIteration && $.def !== missing) {
          return $.def;
        }
    ...
    throw _b_.TypeError.$factory("'" + $B.class_name(obj) + "' object is not an iterator");
  }
  ...
  function ord(c) {
    ...
    switch ($B.get_class(c)) {
      ...
      default:
        throw _b_.TypeError.$factory('ord() expected a character, but ' + $B.class_name(c) + ' was found');
    }
  }
  ...
  function $print() {
    ...
    res = res.replace(new RegExp("\x07", "g"), "").replace(new RegExp("(.)\b", "g"), "");
  ...
  function round() {
    ...
    if (!isinstance(arg, [_b_.int, _b_.float])) {
      ...
      try {
        ...
        if (err.__class__ === _b_.AttributeError) {
          throw _b_.TypeError.$factory("type " + $B.class_name(arg) + " doesn't define __round__ method");
        } else {
          throw err;
        }
      }
    ...
    if (!isinstance(n, _b_.int)) {
      throw _b_.TypeError.$factory("'" + $B.class_name(n) + "' object cannot be interpreted as an integer");
    }
  ...
  $B.$setattr = function (obj, attr, value) {
    ...
    if ($B.aliased_names[attr]) {
      ...
      if (!isinstance(value, _b_.dict)) {
        throw _b_.TypeError.$factory("__dict__ must be set to a dictionary, " + "not a '" + $B.class_name(value) + "'");
      }
      ...
      if (value.__class__) {
        if (value.__module__ == "builtins") {
          error("__class__ assignement only " + "supported for heap types or ModuleType subclasses");
        } else if (Array.isArray(value.__bases__)) {
          for (var i = 0; i < value.__bases__.length; i++) {
            if (value.__bases__[i].__module__ == "builtins") {
              error("__class__ assignment: '" + $B.class_name(obj) + "' object layout differs from '" + $B.class_name(value) + "'");
            }
          }
        }
      }
    ...
    if (obj.$factory || obj.$is_class) {
      ...
      if ($test) {
        console.log("obj is class", metaclass, metaclass[attr]);
      }

      if (metaclass && metaclass[attr] && metaclass[attr].__get__ && metaclass[attr].__set__) {
    ...
    if (klass && klass.__slots__ && special_attrs.indexOf(attr) == -1 && !_setattr) {
      var mangled_slots = function mangled_slots(klass) {
        if (klass.__slots__) {
      ...
      if (!has_slot) {
        throw _b_.AttributeError.$factory("'" + klass.$infos.__name__ + "' object has no attribute '" + attr + "'");
      }
    }
  ...
  $$super.__getattribute__ = function (self, attr) {
    ...
    if ($test) {
      console.log("super", attr, self, f, f + '');
    }
    ...
    if (f.$type == "staticmethod") {
      ...
      var method = function method() {
        var res = callable.apply(void 0, [self.__self_class__].concat(Array.prototype.slice.call(arguments)));
  ...
  $Reader.read = function () {
    ...
    if (self.$binary) {
      res = _b_.bytes.$factory(self.$bytes.source.slice(self.$counter, self.$counter + size));
    } else {
  ...
  $Reader.readline = function (self, size) {
    ...
    if (self.$binary) {
      ...
      if (ix == -1) {
        ...
        var res = {
          __class__: _b_.bytes,
          source: self.$bytes.source.slice(self.$counter, ix + 1)
        };
        self.$counter = ix + 1;
        return res;
      }
  ...
  $Reader.readlines = function () {
    ...
    if (hint < 0) {
      ...
      while (self.$lc < self.$lines.length && nb_read < hint) {
        self.$lc++;
  ...
  function $url_open() {
    ...
    if (isinstance(file, _b_.str)) {
      ...
      if ($B.file_cache.hasOwnProperty($ns.file)) {
        ...
        try {
          for (var _iterator = $res[Symbol.iterator](), _step2; !(_iteratorNormalCompletion = (_step2 = _iterator.next()).done); _iteratorNormalCompletion = true) {
      ...
      throw _b_.TypeError.$factory("invalid argument for open(): " + _b_.str.$factory(file));
    }
  }
  ...
  function no_set_attr(klass, attr) {
    if (klass[attr] !== undefined) {
      throw _b_.AttributeError.$factory("'" + klass.$infos.__name__ + "' object attribute '" + attr + "' is read-only");
    } else {
      throw _b_.AttributeError.$factory("'" + klass.$infos.__name__ + "' object has no attribute '" + attr + "'");
    }
  }
    ...
    switch ($B.$comps[$key]) {
      ...
      case 'lt':
        ellipsis['__' + $B.$comps[$key] + '__'] = function (k) {
          return function (other) {
            throw _b_.TypeError.$factory("unorderable types: ellipsis() " + k + " " + $B.class_name(other));
          };
        }($key);
  ...
  $B.Function = {
    __class__: _b_.type,
    __code__: {
      __class__: FunctionCode,
      __name__: 'function code'
    },
    __globals__: {
      __class__: FunctionGlobals,
      __name__: 'function globals'
    },
    __mro__: [object],
    $infos: {
      __name__: 'function',
  ...
  $B.Function.__get__ = function (self, obj) {
    ...
    var method = function method() {
      return self.apply(void 0, [obj].concat(Array.prototype.slice.call(arguments)));
    };
  ...
  $B.Function.__getattribute__ = function (self, attr) {
    ...
    } else if (self.$infos && self.$infos.__dict__ && self.$infos.__dict__.$string_dict[attr] !== undefined) {
  ...
  $B.Function.__setattr__ = function (self, attr, value) {
    if (attr == "__closure__") {
      ...
      if (value === _b_.None) {
        ...
        throw _b_.TypeError.$factory("__defaults__ must be set to a tuple object");
      }
  ...
  var wrapper_descriptor = $B.wrapper_descriptor = $B.make_class("wrapper_descriptor");
  ...
  wrapper_descriptor.__repr__ = wrapper_descriptor.__str__ = function (self) {
    return "<slot wrapper '" + self.$infos.__name__ + "' of '" + self.__objclass__.$infos.__name__ + "' object>";
  };
...
(function ($B) {
  ...
  $B.$syntax_err_line = function (exc, module, src, pos, line_num) {
    ...
    if (src === undefined) {
      ...
      exc.args = _b_.tuple.$factory([$B.$getitem(exc.args, 0), module, line_num, 0, 0]);
    } else {
      var line_pos = {
        1: 0
      };
  ...
  var traceback = $B.traceback = $B.make_class("traceback", function (exc, stack) {
    var frame = $B.last($B.frames_stack);
  ...
  traceback.__getattribute__ = function (self, attr) {
    ...
    if (attr === 'tb_frame' || attr === 'tb_lineno' || attr === 'tb_lasti' || attr === 'tb_next') {
    ...
    switch (attr) {
      ...
      case "tb_lineno":
        ...
        if (line_info === undefined || first_frame[0].startsWith($B.lambda_magic)) {
          if (first_frame[4] && first_frame[4].$infos && first_frame[4].$infos.__code__) {
  ...
  var frame = $B.make_class("frame", function (stack, pos) {
    var fs = stack;
    var res = {
      __class__: frame,
      f_builtins: {},
    ...
    if (fs.length) {
      ...
      var co_name = locals_id.startsWith("$exec") ? "<string>" : locals_id;
        ...
        if (_frame[1].$name) {
          ...
          if (_frame[4].$infos === undefined) {
            ...
            if (_frame[4].name.startsWith("__ge")) {
              co_name = "<genexpr>";
            } else if (_frame[4].name.startsWith("set_comp" + $B.lambda_magic)) {
              co_name = "<setcomp>";
            }
  ...
  frame.__str__ = frame.__repr__ = function (self) {
    return '<frame object, file ' + self.f_code.co_filename + ', line ' + self.f_lineno + ', code ' + self.f_code.co_name + '>';
  };
  ...
  var getExceptionTrace = function getExceptionTrace(exc, includeInternal) {
      ...
      if (!includeInternal) {
        ...
        if (src === undefined) {
          if ($B.VFS && $B.VFS.hasOwnProperty(frame[2])) {
            src = $B.VFS[frame[2]][1];
          } else if (src = $B.file_cache[frame[3].__file__]) {} else {
            continue;
          }
        }
      }
  ...
  BaseException.__getattr__ = function (self, attr) {
    if (attr == "info") {
      ...
      throw _b_.AttributeError.$factory(self.__class__.$infos.__name__ + " has no attribute '" + attr + "'");
    }
  };
  ...
  var show_stack = $B.show_stack = function (stack) {
    ...
    try {
      for (var _iterator2 = stack[Symbol.iterator](), _step3; !(_iteratorNormalCompletion2 = (_step3 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
        var _frame2 = _step3.value;
        console.log(_frame2[0], _frame2[1].$line_info);
      }
    } catch (err) {
  ...
  $B.exception = function (js_exc, in_ctx_manager) {
    if (!js_exc.__class__) {
      ...
      var $message = "<Javascript " + js_exc.name + ">: " + (js_exc.message || "<" + js_exc + ">");
  ...
  function $make_exc(names, parent) {
    ...
    for (var i = 0; i < names.length; i++) {
      ...
      _str[pos++] = "_b_." + name + ' = {__class__:_b_.type, ' + '__mro__: [_b_.' + parent.$infos.__name__ + "].concat(parent.__mro__), $is_class: true," + "$infos: {__name__:'" + name + "'}}";
      _str[pos++] = "_b_." + name + ".$factory = " + $exc;
      _str[pos++] = "_b_." + name + '.$factory.$infos = {__name__: "' + name + '", __qualname__: "' + name + '"}';
...
(function ($B) {
  ...
  range.__getitem__ = function (self, rank) {
    ...
    if ($B.gt(self.step, 0) && ($B.ge(res, self.stop) || $B.gt(self.start, res)) || $B.gt(0, self.step) && ($B.ge(self.stop, res) || $B.gt(res, self.start))) {
  ...
  range.__next__ = function (self) {
    if (self.$safe) {
      ...
      if (self.step > 0 && self.$counter >= self.stop || self.step < 0 && self.$counter <= self.stop) {
        throw _b_.StopIteration.$factory("");
      }
    } else {
      self.$counter = $B.add(self.$counter, self.step);

      if ($B.gt(self.step, 0) && $B.ge(self.$counter, self.stop) || $B.gt(0, self.step) && $B.ge(self.stop, self.$counter)) {
        throw _b_.StopIteration.$factory("");
      }
    }
  ...
  range.index = function (self, other) {
    ...
    if ($B.eq(res, sub)) {
      if ($B.gt(self.stop, self.start) && $B.ge(other, self.start) && $B.gt(self.stop, other) || $B.ge(self.start, self.stop) && $B.ge(self.start, other) && $B.gt(other, self.stop)) {
        return fl;
      } else {
        throw _b_.ValueError.$factory(_b_.str.$factory(other) + ' not in range');
      }
  ...
  slice.__repr__ = slice.__str__ = function (self) {
    return "slice(" + _b_.str.$factory(self.start) + ", " + _b_.str.$factory(self.stop) + ", " + _b_.str.$factory(self.step) + ")";
  };
    ...
    for (var i = 0; i < attrs.length; i++) {
      ...
      if (val === _b_.None) {
        ...
        try {
          ...
          throw _b_.TypeError.$factory("slice indices must be " + "integers or None or have an __index__ method");
        }
      }
    }
  ...
  slice.indices = function (self, length) {
    ...
    if (_step < 0) {
      var _start = self.start,
          _stop = self.stop;
      _start = _start == _b_.None ? len - 1 : _start < 0 ? _b_.max(-1, _start + len) : _b_.min(len - 1, self.start);
      _stop = self.stop == _b_.None ? -1 : _stop < 0 ? _b_.max(-1, _stop + len) : _b_.min(len - 1, self.stop);
    } else {
...
(function ($B) {
  ...
  bytearray.__setitem__ = function (self, arg, value) {
    if (isinstance(arg, _b_.int)) {
      ...
      throw _b_.TypeError.$factory('list indices must be integer, not ' + $B.class_name(arg));
    }
  };

  bytearray.append = function (self, b) {
    if (arguments.length != 2) {
      throw _b_.TypeError.$factory("append takes exactly one argument (" + (arguments.length - 1) + " given)");
    }
  ...
  bytearray.insert = function (self, pos, b) {
    if (arguments.length != 3) {
      throw _b_.TypeError.$factory("insert takes exactly 2 arguments (" + (arguments.length - 1) + " given)");
    }
  ...
  bytes.__add__ = function (self, other) {
    ...
    throw _b_.TypeError.$factory("can't concat bytes to " + _b_.str.$factory(other));
  };
  ...
  bytes.__getitem__ = function (self, arg) {
    ...
    if (isinstance(arg, _b_.int)) {
      ...
      if (step > 0) {
        ...
        var start = arg.start === None ? getattr(self.source, '__len__')() - 1 : arg.start;
  ...
  bytes.$new = function (cls, source, encoding, errors) {
    ...
    if (source === undefined) {} else if (isinstance(source, _b_.int)) {
      var i = source;
      ...
      if (isinstance(source, _b_.str)) {
        ...
        for (var i = 0; i < int_list.length; i++) {
          try {
            ...
            throw _b_.TypeError.$factory("'" + $B.class_name(int_list[i]) + "' object " + "cannot be interpreted as an integer");
          }

          if (item < 0 || item > 255) {
            throw _b_.ValueError.$factory("bytes must be in range" + "(0, 256)");
          }
        }
      }
    }
  ...
  bytes.count = function () {
    ...
    if (typeof $.sub == "number") {
      if ($.sub < 0 || $.sub > 255) throw _b_.ValueError.$factory("byte must be in range(0, 256)");
      len = 1;
    } else if (!$.sub.__class__) {
      throw _b_.TypeError.$factory("first argument must be a bytes-like " + "object, not '" + $B.class_name($.sub) + "'");
  ...
  bytes.endswith = function () {
    ...
    if (_b_.isinstance($.suffix, bytes)) {
      var start = $.start == -1 ? $.self.source.length - $.suffix.source.length : Math.min($.self.source.length - $.suffix.source.length, $.start);
      var end = $.end == -1 ? $.start == -1 ? $.self.source.length : start + $.suffix.source.length : Math.min($.self.source.length - 1, $.end);
      var res = true;
      ...
      for (var i = 0; i < $.suffix.length; ++i) {
        if (_b_.isinstance($.suffix[i], bytes)) {
          ...
          throw _b_.TypeError.$factory("endswith first arg must be " + "bytes or a tuple of bytes, not " + $B.class_name($.suffix));
        }
      }
  ...
  bytes.expandtabs = function () {
    var $ = $B.args('expandtabs', 2, {
      self: null,
      tabsize: null
    }, ['self', 'tabsize'], arguments, {
      tabsize: 8
    }, null, null);
    var tab_spaces = [];

    for (var _i2 = 0; _i2 < $.tabsize; ++_i2) {
      tab_spaces.push(32);
    }

    var buffer = $.self.source.slice();

    for (var _i3 = 0; _i3 < buffer.length; ++_i3) {
      if (buffer[_i3] === 9) {
        buffer.splice.apply(buffer, [_i3, 1].concat(tab_spaces));
      }
    }
  ...
  bytes.fromhex = function () {
    ...
    for (var i = 0; i < string.length; i += 2) {
      if (i + 2 > string.length) {
        throw _b_.ValueError.$factory("non-hexadecimal number found " + "in fromhex() arg");
      }
  ...
  bytes.isdigit = function (self) {
    ...
    for (var _i4 = 0; _i4 < len && res; ++_i4) {
      res = src[_i4] > 47 && src[_i4] < 58;
    }
  ...
  bytes.islower = function (self) {
    ...
    for (var _i5 = 0; _i5 < len; ++_i5) {
      res = res || src[_i5] > 96 && src[_i5] < 123;

      if (src[_i5] > 64 && src[_i5] < 91) {
        return false;
      }
    }
  ...
  bytes.isupper = function (self) {
    ...
    for (var _i7 = 0; _i7 < len; ++_i7) {
      res = res || src[_i7] > 64 && src[_i7] < 91;

      if (src[_i7] > 96 && src[_i7] < 123) {
        return false;
      }
    }
  ...
  var _lower = function _lower(char_code) {
  ...
  bytes.ljust = function () {
    ...
    if (!$.fillbyte.__class__) {
      throw _b_.TypeError.$factory("argument 2 must be a byte string of length 1, " + "not '" + $B.class_name($.fillbyte) + "'");
  ...
  bytes.partition = function () {
    var $ = $B.args('partition', 2, {
      self: null,
      sep: null
    }, ['self', 'sep'], arguments, {}, null, null);

    if (!$.sep.__class__) {
      throw _b_.TypeError.$factory("a bytes-like object is required, " + "not '" + $B.class_name($.sep) + "'");
  ...
  bytes.replace = function () {
    ...
    if (!$.old.__class__) {
      throw _b_.TypeError.$factory("first argument must be a bytes-like " + "object, not '" + $B.class_name($.old) + "'");
    } else if (!$.old.__class__.$buffer_protocol) {
      throw _b_.TypeError.$factory("first argument must be a bytes-like " + "object, not '" + $B.class_name($.sep) + "'");
    }

    if (!$.new.__class__) {
      throw _b_.TypeError.$factory("second argument must be a bytes-like " + "object, not '" + $B.class_name($.old) + "'");
    } else if (!$.new.__class__.$buffer_protocol) {
      throw _b_.TypeError.$factory("second argument must be a bytes-like " + "object, not '" + $B.class_name($.sep) + "'");
    }
    ...
    if (_b_.isinstance($.prefix, bytes)) {
      ...
      for (var i = 0; i < $.prefix.length; i++) {
        if (_b_.isinstance($.prefix[i], bytes)) {
          ...
          throw _b_.TypeError.$factory("startswith first arg must be " + "bytes or a tuple of bytes, not " + $B.class_name($.prefix));
        }
      }
  ...
  var _upper = function _upper(char_code) {
  ...
  function $UnicodeEncodeError(encoding, code_point, position) {
    throw _b_.UnicodeEncodeError.$factory("'" + encoding + "' codec can't encode character " + _b_.hex(code_point) + " in position " + position);
  }

  function $UnicodeDecodeError(encoding, position) {
    throw _b_.UnicodeDecodeError.$factory("'" + encoding + "' codec can't decode bytes in position " + position);
  }
          ...
          if (!(byte & 0x80)) {
            ...
            if (err_info !== null) {
              if (errors == "ignore") {
                ...
                throw _b_.UnicodeDecodeError.$factory("'utf-8' codec can't decode byte 0x" + err_info[0].toString(16) + "  in position " + err_info[1] + (err_info[2] == "end" ? ": unexpected end of data" : ": invalid continuation byte"));
              }
            } else {
              var cp = byte & 0x1f;
              cp <<= 6;
            ...
            if (errors == "ignore") {
              ...
              throw _b_.UnicodeDecodeError.$factory("'utf-8' codec can't decode byte 0x" + byte.toString(16) + "in position " + pos + ": invalid start byte");
            }
          }
        }
      ...
      case "ascii":
        for (var i = 0, len = b.length; i < len; i++) {
          ...
          if (cp <= 127) {
            ...
            if (errors == "ignore") {} else {
              var msg = "'ascii' codec can't decode byte 0x" + cp.toString(16) + " in position " + i + ": ordinal not in range(128)";
      ...
      case "utf8":
        ...
        for (var i = 0, len = s.length; i < len; i++) {
          ...
          if (cp < 0x7f) {
            ...
            res.push(0xc0 + (cp >> 6), 0x80 + (cp & 0x3f));
          } else if (cp < 0xffff) {
            res.push(0xe0 + (cp >> 12), 0x80 + ((cp & 0xfff) >> 6), 0x80 + (cp & 0x3f));
          } else {
            console.log("4 bytes");
          }
        }
...
(function ($B) {
  ...
  set.__add__ = function (self, other) {
    throw _b_.TypeError.$factory("unsupported operand type(s) for +: 'set' and " + _typeof(other));
  };
  ...
  set.__reduce__ = function (self) {
    return _b_.tuple.$factory([self.__class__, _b_.tuple.$factory([self.$items]), $N]);
  };
  ...
  function $test(accept_iter, other, op) {
    if (accept_iter === undefined && !_b_.isinstance(other, [set, frozenset])) {
      throw _b_.TypeError.$factory("unsupported operand type(s) for " + op + ": 'set' and '" + $B.class_name(other) + "'");
    }
  }
  ...
  function $add(self, item) {
    ...
    if (typeof item === "string" || typeof item === "number" || item instanceof Number) {
      $simple = true;
    }
    ...
    if ($simple) {
      ...
      if (ix == -1) {
        if (item instanceof Number && self.$numbers.indexOf(item.valueOf()) > -1) {} else if (typeof item == "number" && self.$numbers.indexOf(item) > -1) {} else {
  ...
  set.difference_update = function (self) {
    var $ = $B.args("difference_update", 1, {
      self: null
  ...
  set.intersection_update = function () {
    var $ = $B.args("intersection_update", 1, {
      self: null
    }, ["self"], arguments, {}, "args", null),
        self = $.self;
...
(function ($B) {
  ...
  $B.pyobj2structuredclone = function (obj) {
    if (typeof obj == "boolean" || typeof obj == "number" || typeof obj == "string") {
      ...
      return obj.valueOf();
    } else if (Array.isArray(obj) || obj.__class__ === _b_.list || obj.__class__ === _b_.tuple) {
      var res = [];
      ...
      if (Object.keys(obj.$numeric_dict).length > 0 || Object.keys(obj.$object_dict).length > 0) {
        throw _b_.TypeError.$factory("a dictionary with non-string " + "keys cannot be sent to or from a Web Worker");
      }
  ...
  $B.structuredclone2pyobj = function (obj) {
    if (obj === null) {
      ...
      throw _b_.TypeError.$factory(_b_.str.$factory(obj) + " does not support the structured clone algorithm");
    }
  };
  ...
  var UndefinedClass = $B.make_class("undefined", function () {
    return Undefined;
  });

  UndefinedClass.__bool__ = function () {
    return false;
  };

  UndefinedClass.__repr__ = function () {
    return "undefined";
  };
  ...
  var pyobj2jsobj = $B.pyobj2jsobj = function (pyobj) {
    ...
    if (klass === JSObject || klass === JSConstructor) {
      ...
      return pyobj.js;
    } else if (klass === $B.DOMNode || klass.__mro__.indexOf($B.DOMNode) > -1) {
      return pyobj.elt;
  ...
  JSObject.__getattribute__ = function (self, attr) {
    ...
    if ($test) {
      console.log("get attr", attr, "of", self);
    }
    ...
    if (self.js === null) {
      return object.__getattribute__(None, attr);
    }
    ...
    if (self.__class__ === JSObject && attr == "bind" && self.js[attr] === undefined && self.js['addEventListener'] !== undefined) {
    ...
    if (js_attr !== undefined) {
      ...
      if (typeof js_attr == 'function') {
        var res = function res() {
          ...
          for (var i = 0, len = arguments.length; i < len; i++) {
            ...
            if (arg !== undefined && arg !== null && arg.$nat !== undefined) {
              ...
              if (Array.isArray(kw)) {
                kw = $B.extend.apply($B, [js_attr.name].concat(_toConsumableArray(kw)));
              }

              if (Object.keys(kw).length > 0) {
                throw _b_.TypeError.$factory("A Javascript function can't take " + "keyword arguments");
              }
            } else {
        ...
        if ($test) {
          console.log("use JS2Py", $B.$JS2Py(js_attr));
        }
    ...
    if (res !== undefined) {
      if ($test) {
        console.log("found in klass", res + "");
      }
      ...
      if (typeof res === 'function') {
        return function () {
          ...
          for (var i = 0, len = arguments.length; i < len; i++) {
            ...
            if (arg && (arg.__class__ === JSObject || arg.__class__ === JSConstructor)) {
              args.push(arg.js);
            } else {
              args.push(arg);
            }
  ...
  JSObject.__getitem__ = function (self, rank) {
    if (typeof self.js.length == 'number') {
      if ((typeof rank == "number" || typeof rank == "boolean") && typeof self.js.item == 'function') {
        var rank_to_int = _b_.int.$factory(rank);

        if (rank_to_int < 0) {
          rank_to_int += self.js.length;
        }
        ...
        return JSObject.$factory(res);
      } else if (typeof rank == "string" && typeof self.js.getNamedItem == 'function') {
  ...
  JSObject.__len__ = function (self) {
    ...
    try {
      return getattr(self.js, '__len__')();
    } catch (err) {
          ...
          try {
            ...
            if (err.args.length > 0) {
              err.toString = function () {
                return info + '\n' + $B.class_name(err) + ': ' + _b_.repr(err.args[0]);
              };
            } else {
              err.toString = function () {
                return info + '\n' + $B.class_name(err);
  ...
  JSObject.bind = function (self, evt, func) {
    var js_func = function js_func(ev) {
  ...
  JSObject.$factory = function (obj) {
    ...
    if (klass === _b_.float) {
      return _b_.float.$factory(obj);
    }

    if (klass === _b_.list) {
      return $B.JSArray.$factory(obj);
    }
...
(function ($B) {
  ...
  function $download_module(mod, url, $package) {
    ...
    if ($B.$CORS) {
      ...
      if (xhr.readyState == 4) {
        if (xhr.status == 200) {
          ...
          console.info("Error " + xhr.status + " means that Python module " + mod_name + " was not found at url " + url);
          res = _b_.FileNotFoundError.$factory("No module named '" + mod_name + "'");
        }
      }
    }
    ...
    if (res == null) {
      throw _b_.FileNotFoundError.$factory("No module named '" + mod_name + "' (res is null)");
    }
  ...
  var finder_VFS = {
    ...
    create_module: function create_module(cls, spec) {
      return _b_.None;
    },
    exec_module: function exec_module(cls, modobj) {
      ...
      if (ext == '.js') {
        ...
        for (var i = 0; i < parts.length; i++) {
          ...
          if ($B.imported.hasOwnProperty(parent) && $B.imported[parent].__initialized__) {
            continue;
          }
          ...
          try {
            ...
            var $module = new Function("$locals_" + parent_id, mod_js)(mod);
          } catch (err) {
            if ($B.debug > 1) {
              console.log(err);
        ...
        $B.precompiled[mod_name] = record.is_package ? [record.content] : record.content;
        ...
        if ($B.$options.indexedDB && self.indexedDB && $B.idb_name) {
    ...
    find_module: function find_module(cls, name, path) {
      return {
        __class__: Loader,
        load_module: function load_module(name, path) {
    ...
    find_spec: function find_spec(cls, fullname, path, prev_module) {
  ...
  for (var method in finder_VFS) {
    if (typeof finder_VFS[method] == "function") {
      finder_VFS[method] = _b_.classmethod.$factory(finder_VFS[method]);
    }
  }

  finder_VFS.$factory = function () {
    return {
      __class__: finder_VFS
    };
  };
  ...
  var finder_stdlib_static = {
    ...
    exec_module: function exec_module(cls, module) {
      ...
      if ($B.stdlib && $B.$options.static_stdlib_import) {
        ...
        if (address !== undefined) {
          var ext = address[0],
              ...
              path = $B.brython_path + (ext == "py" ? "Lib/" : "libs/") + fullname.replace(/\./g, "/"),
              metadata = {
            ext: ext,
            is_package: is_pkg,
            path: path + (is_pkg ? "/__init__.py" : ext == "py" ? ".py" : ".js"),
            address: address
          };
          var res = new_spec({
            name: fullname,
  ...
  for (var method in finder_stdlib_static) {
    if (typeof finder_stdlib_static[method] == "function") {
      finder_stdlib_static[method] = _b_.classmethod.$factory(finder_stdlib_static[method]);
    }
  }

  finder_stdlib_static.$factory = function () {
    return {
      __class__: finder_stdlib_static
    };
  };
  ...
  var finder_path = {
    ...
    exec_module: function exec_module(cls, _module) {
    ...
    find_spec: function find_spec(cls, fullname, path, prev_module) {
      ...
      for (var i = 0, li = path.length; i < li; ++i) {
        ...
        if (finder === undefined) {
          ...
          for (var j = 0, lj = $B.path_hooks.length; j < lj && finder_notfound; ++j) {
            var hook = $B.path_hooks[j].$factory;

            try {
              finder = (typeof hook == "function" ? hook : $B.$getattr(hook, "__call__"))(path_entry);
        ...
        var find_spec = $B.$getattr(finder, "find_spec"),
            fs_func = typeof find_spec == "function" ? find_spec : $B.$getattr(find_spec, "__call__");
  ...
  for (var method in finder_path) {
    if (typeof finder_path[method] == "function") {
      finder_path[method] = _b_.classmethod.$factory(finder_path[method]);
    }
  }

  finder_path.$factory = function () {
    return {
      __class__: finder_path
    };
  };
  ...
  var url_hook = {
    ...
    __mro__: [_b_.object],
    __repr__: function __repr__(self) {
      return "<UrlPathFinder" + (self.hint ? " for '" + self.hint + "'" : "(unbound)") + " at " + self.path_entry + '>';
    },
    $infos: {
      __module__: "builtins",
      __name__: "UrlPathFinder"
    },
    find_spec: function find_spec(self, fullname, module) {
      var loader_data = {},
          notfound = true,
          hint = self.hint,
      ...
      for (var j = 0; notfound && j < modpaths.length; ++j) {
        try {
          ...
          if (loader_data.is_package) {
            $B.path_importer_cache[base_path + '/'] = url_hook.$factory(base_path + '/', self.hint);
          }
      ...
      if (!notfound) {
        return new_spec({
          ...
          submodule_search_locations: loader_data.is_package ? [base_path] : _b_.None,
    ...
    invalidate_caches: function invalidate_caches(self) {}
  };
  ...
  $B.$__import__ = function (mod_name, globals, locals, fromlist, level) {
    ...
    if (globals.$jsobj && globals.$jsobj.__file__) {
      ...
      if (file.startsWith($B.brython_path + "Lib/") && !file.startsWith($B.brython_path + "Lib/site-packages/") || file.startsWith($B.brython_path + "libs/") || file.startsWith("VFS.")) {
        from_stdlib = true;
      }
    }
    ...
    if (modobj === undefined) {
      ...
      for (var i = 0, modsep = "", _mod_name = "", len = parsed_name.length - 1, __path__ = _b_.None; i <= len; ++i) {
        ...
        if (i < len) {
          try {
            ...
            if (i == len - 1 && $B.imported[_mod_name][parsed_name[len]] && $B.imported[_mod_name][parsed_name[len]].__class__ === module) {
      ...
      if ($B.imported[parsed_name[0]] && parsed_name.length > 1) {
        try {
  ...
  $B.$import = function (mod_name, fromlist, aliases, locals) {
    ...
    for (var i = 0, len = parts.length; i < len; i++) {
      ...
      if (prefix && p == "") {
        ...
        if (elt === undefined) {
          throw _b_.ImportError.$factory("Parent module '' not loaded, " + "cannot perform relative import");
        }
      } else {
        prefix = false;
    ...
    var importer = typeof __import__ == "function" ? __import__ : $B.$getattr(__import__, "__call__"),
          ...
          try {
            ...
            try {
              ...
              if ($err3.$py_error) {
                ...
                if ($err3.__class__ !== _b_.ImportError && $err3.__class__ !== _b_.ModuleNotFoundError) {
                  $B.handle_error($err3);
                }
...
(function ($B) {
  ...
  function $err(op, other) {
    var msg = "unsupported operand type(s) for " + op + ": 'float' and '" + $B.class_name(other) + "'";
    throw _b_.TypeError.$factory(msg);
  }
  ...
  float.as_integer_ratio = function (self) {
    ...
    if (self.valueOf() == Number.POSITIVE_INFINITY || self.valueOf() == Number.NEGATIVE_INFINITY) {
  ...
  float.fromhex = function (arg) {
    ...
    if (_m !== null) {
      return $FloatClass(parseFloat(_m[1]));
    }
  ...
  function preformat(self, fmt) {
    ...
    if (fmt.type && 'eEfFgGn%'.indexOf(fmt.type) == -1) {
      throw _b_.ValueError.$factory("Unknown format code '" + fmt.type + "' for object of type 'float'");
    }
    ...
    if (fmt.type == "e") {
      ...
      if (Math.abs(exp) < 10) {
        res = res.substr(0, res.length - 1) + "0" + res.charAt(res.length - 1);
      }
    ...
    if (fmt.precision !== undefined) {
      ...
      if (fmt.type !== undefined && (fmt.type == "%" || fmt.type.toLowerCase() == "f")) {
        if (pt_pos == -1) {
  ...
  float.__mod__ = function (self, other) {
    ...
    if (hasattr(other, "__rmod__")) {
      return getattr(other, "__rmod__")(self);
    }
  ...
  float.__mul__ = function (self, other) {
    ...
    if (isinstance(other, _b_.complex)) {
      return $B.make_complex(float.$factory(self * other.$real), float.$factory(self * other.$imag));
    }

    if (hasattr(other, "__rmul__")) {
      return getattr(other, "__rmul__")(self);
    }
  ...
  float.__pow__ = function (self, other) {
    ...
    if (other_int || isinstance(other, float)) {
      ...
      if (self == -1 && (!isFinite(other) || other.__class__ === $B.long_int || !$B.is_safe_int(other)) && !isNaN(other)) {
        return new Number(1);
    ...
    if (hasattr(other, "__rpow__")) {
      return getattr(other, "__rpow__")(self);
    }
  ...
  var $op_func = function $op_func(self, other) {
    ...
    if (hasattr(other, "__rsub__")) {
      return getattr(other, "__rsub__")(self);
    }
  ...
  var $comp_func = function $comp_func(self, other) {
    ...
    throw _b_.TypeError.$factory("unorderable types: float() > " + $B.class_name(other) + "()");
  };

  $comp_func += "";

  for (var $op in $B.$comps) {
    eval("float.__" + $B.$comps[$op] + "__ = " + $comp_func.replace(/>/gm, $op).replace(/__gt__/gm, "__" + $B.$comps[$op] + "__").replace(/__le__/, "__" + $B.$inv_comps[$op] + "__"));
  }

  $B.make_rmethods(float);

  var $notimplemented = function $notimplemented(self, other) {
    throw _b_.TypeError.$factory("unsupported operand types for OPERATOR: 'float' and '" + $B.class_name(other) + "'");
  };
  ...
  for (var $op in $B.$operators) {
    if ($B.augmented_assigns[$op] === undefined) {
      ...
      if (float[$opfunc] === undefined) {
        eval("float." + $opfunc + "=" + $notimplemented.replace(/OPERATOR/gm, $op));
      }
    }
  }
  ...
  float.$factory = function (value) {
    ...
    if (typeof value == "string") {
      ...
      switch (value.toLowerCase()) {
        ...
        default:
          ...
          if (isFinite(value)) return $FloatClass(eval(value));else {
            _b_.str.encode(value, "latin-1");

            throw _b_.ValueError.$factory("Could not convert to float(): '" + _b_.str.$factory(value) + "'");
          }
      }
    }
    ...
    throw _b_.TypeError.$factory("float() argument must be a string or a " + "number, not '" + $B.class_name(value) + "'");
  };
...
(function ($B) {
  ...
  function $err(op, other) {
    var msg = "unsupported operand type(s) for " + op + ": 'int' and '" + $B.class_name(other) + "'";
    throw _b_.TypeError.$factory(msg);
  }
  ...
  int.to_bytes = function () {
    var $ = $B.args("to_bytes", 3, {
      ...
      len: null,
      byteorder: null
    }, ["self", "len", "byteorder"], arguments, {}, "args", "kw"),
        self = $.self,
        len = $.len,
        byteorder = $.byteorder,
        kwargs = $.kw;
    ...
    if (!_b_.isinstance(len, _b_.int)) {
      throw _b_.TypeError.$factory("integer argument expected, got " + $B.class_name(len));
    }
  ...
  function preformat(self, fmt) {
    ...
    if (fmt.type && 'bcdoxXn'.indexOf(fmt.type) == -1) {
      throw _b_.ValueError.$factory("Unknown format code '" + fmt.type + "' for object of type 'int'");
    }
  ...
  int.__hash__ = function (self) {
    if (self === undefined) {
      return int.__hashvalue__ || $B.$py_next_hash--;
    }
  ...
  int.__mod__ = function (self, other) {
    ...
    if (_b_.isinstance(other, [int, _b_.float, bool])) {
      ...
      if (other == 0) {
        throw _b_.ZeroDivisionError.$factory("integer division or modulo by zero");
      }
  ...
  int.__mul__ = function (self, other) {
    ...
    if (_b_.isinstance(other, _b_.complex)) {
      return $B.make_complex(int.__mul__(self, other.$real), int.__mul__(self, other.$imag));
    }
    ...
    if (_b_.isinstance(other, [_b_.list, _b_.tuple])) {
      ...
      if (_b_.isinstance(other, _b_.tuple)) {
        res = _b_.tuple.$factory(res);
      }
  ...
  int.__pow__ = function (self, other, z) {
    if (_b_.isinstance(other, int)) {
        ...
        while (exponent > 0) {
          if (exponent % 2 == 1) {
            if (result * base > $B.max_int) {
              result = long_int.__mul__(long_int.$factory(result), long_int.$factory(base));
      ...
      if (res > $B.min_int && res < $B.max_int) {
        ...
        return int.$factory($B.long_int.__pow__($B.long_int.$factory(self), $B.long_int.$factory(other)));
      }
    }

    if (_b_.isinstance(other, _b_.float)) {
      if (self >= 0) {
        return new Number(Math.pow(self, other.valueOf()));
      } else {
        return _b_.complex.__pow__($B.make_complex(self, 0), other);
      }
    ...
    if (hasattr(other, "__rpow__")) {
      return $B.$getattr(other, "__rpow__")(self);
    }
  ...
  for (var $op in $B.$comps) {
    eval("int.__" + $B.$comps[$op] + "__ = " + $comp_func.replace(/>/gm, $op).replace(/__gt__/gm, "__" + $B.$comps[$op] + "__").replace(/__lt__/, "__" + $B.$inv_comps[$op] + "__"));
  }

  $B.make_rmethods(int);

  var $valid_digits = function $valid_digits(base) {
    var digits = "";
  ...
  int.$factory = function (value, base) {
    ...
    if (typeof value == "number" && (base === undefined || base == 10)) {
    ...
    function invalid(value, base) {
      throw _b_.ValueError.$factory("invalid literal for int() with base " + base + ": '" + _b_.str.$factory(value) + "'");
    }
    ...
    if (typeof value == "string") {
      ...
      var _digits = $valid_digits(base),
          _re = new RegExp("^[+-]?[" + _digits + "]" + "[" + _digits + "_]*$", "i"),
    ...
    if (num_value === null) {
      throw _b_.TypeError.$factory("int() argument must be a string, a bytes-like " + "object or a number, not '" + $B.class_name(value) + "'");
    }
      ...
      default:
        ...
        if (bool_method === missing) {
          ...
          if (res !== true && res !== false) {
            throw _b_.TypeError.$factory("__bool__ should return " + "bool, returned " + $B.class_name(res));
          }
...
(function ($B) {
  ...
  function check_shift(shift) {
    if (!isinstance(shift, long_int)) {
      throw TypeError.$factory("shift must be int, not '" + $B.class_name(shift) + "'");
    }
  ...
  long_int.__and__ = function (self, other) {
    ...
    if (v2.charAt(0) == "1") {
      v2 = "1".repeat(v1.length - v2.length) + v2;
    }
  ...
  long_int.__ge__ = function (self, other) {
    ...
    if (self.value.length > other.value.length) {
      ...
      return self.pos ? self.value >= other.value : self.value <= other.value;
    }
  };
  ...
  long_int.__truediv__ = function (self, other) {
    if (isinstance(other, long_int)) {
      ...
      throw TypeError.$factory("unsupported operand type(s) for /: 'int' and '" + $B.class_name(other) + "'");
    }
  };
  ...
  function isSafeInteger(n) {
    return typeof n === "number" && Math.round(n) === n && MIN_SAFE_INTEGER <= n && n <= MAX_SAFE_INTEGER;
  }
  ...
  long_int.$factory = function (value, base) {
    if (arguments.length > 2) {
      throw _b_.TypeError.$factory("long_int takes at most 2 arguments (" + arguments.length + " given)");
    }

    if (base === undefined) {
      base = 10;
    } else if (!isinstance(base, int)) {
      throw TypeError.$factory("'" + $B.class_name(base) + "' object cannot be interpreted as an integer");
    }
    ...
    if (typeof value == "number") {
      if (isSafeInteger(value)) {
        ...
        throw ValueError.$factory("argument of long_int is not a safe integer");
      }
      ...
      throw ValueError.$factory("argument of long_int must be a string, not " + $B.class_name(value));
    }
...
(function ($B) {
  ...
  complex.__new__ = function (cls) {
    ...
    if (typeof $real == "string") {
      if ($imag !== missing) {
        ...
        var to_num = function to_num(s) {
          var res = parseFloat(s.charAt(0) + s.substr(1).replace(/_/g, ""));

          if (isNaN(res)) {
            throw _b_.ValueError.$factory("could not convert string " + "to complex: '" + arg + "'");
          }
    ...
    if (_b_.isinstance($real, [_b_.float, _b_.int]) && _b_.isinstance($imag, [_b_.float, _b_.int])) {
      res = {
        __class__: complex,
        $real: $real,
        $imag: $imag
    ...
    if (real_to_num === null) {
      throw _b_.TypeError.$factory("complex() first argument must be a " + " string or a number, not '" + $B.class_name($real) + "'");
    }

    $real = real_to_num;
    $imag = _convert($imag);

    if (!_b_.isinstance($real, _b_.float) && !_b_.isinstance($real, _b_.int) && !_b_.isinstance($real, _b_.complex)) {
    ...
    if (!_b_.isinstance($imag, _b_.float) && !_b_.isinstance($imag, _b_.int) && !_b_.isinstance($imag, _b_.complex) && $imag !== missing) {
      throw _b_.TypeError.$factory("complex() argument must be a string " + "or a number");
    }
  ...
  complex.__pow__ = function (self, other) {
    ...
    if (_b_.isinstance(other, [_b_.int, _b_.float])) {
      ...
      throw _b_.TypeError.$factory("unsupported operand type(s) " + "for ** or pow(): 'complex' and '" + $B.class_name(other) + "'");
    }
  };
  ...
  var $op_func = function $op_func(self, other) {
    throw _b_.TypeError.$factory("TypeError: unsupported operand type(s) " + "for -: 'complex' and '" + $B.class_name(other) + "'");
  };
  ...
  var $op_func = function $op_func(self, other) {
    ...
    throw _b_.TypeError.$factory("unsupported operand type(s) for -: " + self.__repr__() + " and '" + $B.class_name(other) + "'");
  };
...
(function ($B) {
  ...
  var TimSortAssertion = function TimSortAssertion(message) {
  ...
  var TimSort = function TimSort(array, compare) {
    var self = {
      ...
      runLength: null,
      stackSize: 0,
      pushRun: function pushRun(runStart, runLength) {
      ...
      mergeLow: function mergeLow(start1, length1, start2, length2) {
        var compare = this.compare,
            array = this.array,
            tmp = this.tmp,
            i = 0;
      ...
      mergeHigh: function mergeHigh(start1, length1, start2, length2) {
        var compare = this.compare,
            array = this.array,
            tmp = this.tmp,
            i = 0;
    ...
    self.stackLength = self.length < 120 ? 5 : self.length < 1542 ? 10 : self.length < 119151 ? 19 : 40;
...
(function ($B) {
  ...
  function check_not_tuple(self, attr) {
    if (self.__class__ === tuple) {
      throw _b_.AttributeError.$factory("'tuple' object has no attribute '" + attr + "'");
    }
  }
  ...
  list.__add__ = function (self, other) {
    if ($B.get_class(self) !== $B.get_class(other)) {
      ...
      if (radd !== _b_.NotImplemented) {
        return radd(self);
      }

      throw _b_.TypeError.$factory('can only concatenate list (not "' + $B.class_name(other) + '") to list');
    }
  ...
  list.__getitem__ = function (self, arg) {
    ...
    throw _b_.TypeError.$factory("list indices must be integer, not " + $B.class_name(key));
  };
  ...
  list.__ge__ = function (self, other) {
    ...
    while (i < self.length) {
      ...
      if ($B.rich_comp("__eq__", self[i], other[i])) {
        ...
        if (res === _b_.NotImplemented) {
          throw _b_.TypeError.$factory("unorderable types: " + $B.class_name(self[i]) + "() >= " + $B.class_name(other[i]) + "()");
        } else {
          return res;
        }
      }
  ...
  list.__mul__ = function (self, other) {
    ...
    throw _b_.TypeError.$factory("can't multiply sequence by non-int of type '" + $B.class_name(other) + "'");
  };
      ...
      if (func === $N) {
        if (reverse) {
          cmp = function cmp(b, a) {
            ...
            if (res === _b_.NotImplemented) {
              throw _b_.TypeError.$factory("unorderable types: " + $B.class_name(b) + "() <=" + $B.class_name(a) + "()");
            }
          ...
          cmp = function cmp(a, b) {
            ...
            if (res === _b_.NotImplemented) {
              throw _b_.TypeError.$factory("unorderable types: " + $B.class_name(a) + "() <=" + $B.class_name(b) + "()");
            }
...
$B.unicode = {
  "Cc": [[0, 32], [127, 33]],
  "Zs": [32, 160, 5760, 6158, [8192, 11], 8239, 8287, 12288],
...
for (var key in $B.unicode_identifiers) {
  ...
  try {
    for (var _iterator3 = $B.unicode_identifiers[key][Symbol.iterator](), _step4; !(_iteratorNormalCompletion3 = (_step4 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
...
(function ($B) {
  ...
  function check_str(obj) {
    if (!_b_.isinstance(obj, str)) {
      throw _b_.TypeError.$factory("can't convert '" + $B.class_name(obj) + "' object to str implicitly");
    }
  }
  ...
  str.__add__ = function (self, other) {
    if (!(typeof other === "string")) {
      try {
        return getattr(other, "__radd__")(self);
      } catch (err) {
        throw _b_.TypeError.$factory("Can't convert " + $B.class_name(other) + " to str implicitly");
      }
    }
  ...
  str.__contains__ = function (self, item) {
    if (!(typeof item == "string")) {
      throw _b_.TypeError.$factory("'in <string>' requires " + "string as left operand, not " + item.__class__);
    }
  ...
  function preformat(self, fmt) {
    ...
    if (fmt.type && fmt.type != "s") {
      throw _b_.ValueError.$factory("Unknown format code '" + fmt.type + "' for object of type 'str'");
    }
  ...
  var number_check = function number_check(s) {
    if (!isinstance(s, [_b_.int, _b_.float])) {
      throw new NotANumber();
    }
  };

  var get_char_array = function get_char_array(size, char) {
    if (size <= 0) {
      return "";
    }
  ...
  var format_padding = function format_padding(s, flags, minus_one) {
  ...
  var format_int_precision = function format_int_precision(val, flags) {
  ...
  var format_float_precision = function format_float_precision(val, upper, flags, modifier) {
    var precision = flags.precision;
  ...
  var format_sign = function format_sign(val, flags) {
    if (flags.sign) {
      if (val >= 0) {
        return "+";
      }
  ...
  var str_format = function str_format(val, flags) {
    flags.pad_char = " ";
    return format_padding(str.$factory(val), flags);
  };

  var num_format = function num_format(val, flags) {
    number_check(val);
  ...
  var repr_format = function repr_format(val, flags) {
    flags.pad_char = " ";
    return format_padding(repr(val), flags);
  };

  var ascii_format = function ascii_format(val, flags) {
    flags.pad_char = " ";
    return format_padding(ascii(val), flags);
  };

  var _float_helper = function _float_helper(val, flags) {
    number_check(val);
  ...
  var validate_precision = function validate_precision(precision) {
    if (precision > 20) {
      precision = 20;
    }
  };

  var floating_point_format = function floating_point_format(val, upper, flags) {
    val = _float_helper(val, flags), v = val.toString(), v_len = v.length, dot_idx = v.indexOf('.');
    ...
    if (val < 1 && val > -1) {
      ...
      if (numzeros >= 4) {
        ...
        if (!flags.alternate) {
          ...
          if (trl) {
            val = trl[1].replace(trailing_dot, "") + trl[3];
          }
        } else {
      ...
      return format_padding(format_sign(val, flags) + format_float_precision(val, upper, flags, function (val, precision) {
  ...
  var _floating_g_exp_helper = function _floating_g_exp_helper(val, precision, flags, upper) {
    if (precision) {
      --precision;
    }
  ...
  var floating_point_decimal_format = function floating_point_decimal_format(val, upper, flags) {
    val = _float_helper(val, flags);
    return format_padding(format_sign(val, flags) + format_float_precision(val, upper, flags, function (val, precision, flags) {
  ...
  var _floating_exp_helper = function _floating_exp_helper(val, precision, flags, upper) {
    val = val.toExponential(precision);
    var e_idx = val.lastIndexOf("e");
  ...
  var floating_point_exponential_format = function floating_point_exponential_format(val, upper, flags) {
    val = _float_helper(val, flags);
    return format_padding(format_sign(val, flags) + format_float_precision(val, upper, flags, _floating_exp_helper), flags);
  };

  var signed_hex_format = function signed_hex_format(val, upper, flags) {
    var ret;
    number_check(val);
  ...
  var octal_format = function octal_format(val, flags) {
    number_check(val);
    var ret;
  ...
  function series_of_bytes(val, flags) {
    if (val.__class__ && val.__class__.$buffer_protocol) {
      ...
      try {
        ...
        if (err.__class__ === _b_.AttributeError) {
          throw _b_.TypeError.$factory("%b does not accept '" + $B.class_name(val) + "'");
        }
  ...
  var single_char_format = function single_char_format(val, flags) {
  ...
  var num_flag = function num_flag(c, flags) {
  ...
  var decimal_point_flag = function decimal_point_flag(val, flags) {
    if (flags.decimal_point) {
  ...
  var neg_flag = function neg_flag(val, flags) {
    flags.pad_char = " ";
    flags.left = true;
  };

  var space_flag = function space_flag(val, flags) {
    flags.space = true;
  };

  var sign_flag = function sign_flag(val, flags) {
    flags.sign = true;
  };

  var alternate_flag = function alternate_flag(val, flags) {
  ...
  var char_mapping = {
    ...
    "0": function _(val, flags) {
      return num_flag("0", flags);
    },
    "1": function _(val, flags) {
      return num_flag("1", flags);
    },
    "2": function _(val, flags) {
      return num_flag("2", flags);
    },
    "3": function _(val, flags) {
      return num_flag("3", flags);
    },
    "4": function _(val, flags) {
      return num_flag("4", flags);
    },
    "5": function _(val, flags) {
      return num_flag("5", flags);
    },
    "6": function _(val, flags) {
      return num_flag("6", flags);
    },
    "7": function _(val, flags) {
      return num_flag("7", flags);
    },
    "8": function _(val, flags) {
      return num_flag("8", flags);
    },
    "9": function _(val, flags) {
  ...
  str.__mod__ = function (self, args) {
    ...
    var $get_kwarg_string = function $get_kwarg_string(s) {
      ++pos;
    ...
    var $get_arg_string = function $get_arg_string(s) {
      var self;
    ...
    var get_string_value = function get_string_value(s, self) {
      var flags = {
        "pad_char": " "
      };
      ...
      do {
        ...
        try {
          ...
          if (err.name == "UnsupportedChar") {
            ...
            throw _b_.ValueError.$factory("unsupported format character '" + invalid_char + "' (0x" + invalid_char.charCodeAt(0).toString(16) + ") at index " + newpos);
  ...
  str.__repr__ = function (self) {
    var res = self;
    res = self.replace(/\\/g, "\\\\");
    res = res.replace(new RegExp("\x07", "g"), "\\x07").replace(new RegExp("\b", "g"), "\\x08").replace(new RegExp("\x0B", "g"), "\\x0b").replace(new RegExp("\f", "g"), "\\x0c").replace(new RegExp("\n", "g"), "\\n").replace(new RegExp("\r", "g"), "\\r").replace(new RegExp("\t", "g"), "\\t");
  ...
  str.count = function () {
    ...
    if (!(typeof $.sub == "string")) {
      throw _b_.TypeError.$factory("Can't convert '" + $B.class_name($.sub) + "' object to str implicitly");
    }
  ...
  str.endswith = function () {
    ...
    for (var i = 0, len = suffixes.length; i < len; i++) {
      ...
      if (!_b_.isinstance(suffix, str)) {
        throw _b_.TypeError.$factory("endswith first arg must be str or a tuple of str, not int");
      }

      if (suffix.length <= s.length && s.substr(s.length - suffix.length) == suffix) {
        return true;
      }
    }
  ...
  str.find = function () {
    ...
    if ($.sub.length == 0 && $.start == $.self.length) {
      return $.self.length;
    }
  ...
  $B.parse_format = function (fmt_string) {
    ...
    if (name !== undefined) {
      var name_repl = function name_repl(match) {
        name_ext.push(match);
        return "";
      };
  ...
  $B.split_format = function (self) {
    ...
    while (pos < _len) {
      ...
      if (car == "{" && self.charAt(pos + 1) == "{") {
          ...
          if (self.charAt(end) == "{") {
            ...
            if (nb == 0) {
              ...
              if (fmt_obj.spec !== undefined) {
                var replace_nested = function replace_nested(name, key) {
                  if (key == "") {
                    return "{" + rank++ + "}";
                  }
        ...
        if (nb > 0) {
          throw ValueError.$factory("wrong format " + self);
        }
  ...
  str.isalnum = function (self) {
    ...
    for (var i = 0, len = self.length; i < len; i++) {
      ...
      if (unicode_tables.Ll[char] || unicode_tables.Lu[char] || unicode_tables.Lm[char] || unicode_tables.Lt[char] || unicode_tables.Lo[char] || unicode_tables.Nd[char] || unicode_tables.digits[char] || unicode_tables.numeric[char]) {
        continue;
      }
  ...
  str.isalpha = function (self) {
    ...
    for (var i = 0, len = self.length; i < len; i++) {
      ...
      if (unicode_tables.Ll[char] || unicode_tables.Lu[char] || unicode_tables.Lm[char] || unicode_tables.Lt[char] || unicode_tables.Lo[char]) {
        continue;
      }
  ...
  str.isspace = function (self) {
    ...
    for (var i = 0, len = self.length; i < len; i++) {
      ...
      if (!unicode_tables.Zs[char] && $B.unicode_bidi_whitespace.indexOf(char) == -1) {
        return false;
      }
    }
    ...
    while (1) {
      try {
        ...
        if (!isinstance(obj2, str)) {
          throw _b_.TypeError.$factory("sequence item " + count + ": expected str instance, " + $B.class_name(obj2) + " found");
        }
      ...
      for (var i = 0, len = items.length; i < len; i++) {
        ...
        if (!_b_.isinstance(k, _b_.int)) {
          if (_b_.isinstance(k, _b_.str) && k.length == 1) {
            ...
            throw _b_.TypeError.$factory("dictionary key " + k + " is not int or 1-char string");
          }
        }

        if (v !== _b_.None && !_b_.isinstance(v, [_b_.int, _b_.str])) {
          throw _b_.TypeError.$factory("dictionary value " + v + " is not None, integer or string");
        }
  ...
  str.replace = function (self, old, _new, count) {
    ...
    if (!isinstance(count, [_b_.int, _b_.float])) {
      throw _b_.TypeError.$factory("'" + $B.class_name(count) + "' object cannot be interpreted as an integer");
  ...
  str.splitlines = function (self) {
    ...
    if (!_b_.isinstance($.keepends, [_b_.bool, _b_.int])) {
      throw _b_.TypeError.$factory("integer argument expected, got " + $B.get_class($.keepends).__name);
    }
  ...
  str.startswith = function () {
    ...
    }, ["self", "prefix", "start", "end"], arguments, {
      start: 0,
      end: null
    }, null, null);
    normalize_start_end($);
  ...
  str.$factory = function (arg, encoding, errors) {
    ...
    try {
      ...
      if (arg.__class__ && arg.__class__ === _b_.bytes && encoding !== undefined) {
      ...
      if (method === null || arg.__class__ && arg.__class__ !== _b_.object && method.$infos && method.$infos.__func__ === _b_.object.__str__) {
  ...
  $B.parse_format_spec = function (spec) {
    ...
    this.toString = function () {
      return (this.fill === undefined ? "" : _b_.str.$factory(this.fill)) + (this.align || "") + (this.sign || "") + (this.alternate ? "#" : "") + (this.sign_aware ? "0" : "") + (this.width || "") + (this.comma ? "," : "") + (this.precision ? "." + this.precision : "") + (this.type || "");
    };
  };
    ...
    while (pos < string.length) {
      if (ctype === null) {
        ...
        while (i < string.length) {
          ...
          if (car == "{" && nb_paren == 0) {
          ...
          } else if (nb_paren == 0 && car == "!" && current.fmt === null && ":}".indexOf(string.charAt(i + 2)) > -1) {
            ...
            if (ce.length == 0 || string.charAt(i + 1) == "=" || "=!<>:".search(ce.charAt(ce.length - 1)) > -1) {
...
(function ($B) {
  ...
  dict.__contains__ = function () {
    ...
    if (self.$str_hash[hash] !== undefined && $B.rich_comp("__eq__", key, self.$str_hash[hash])) {
      return true;
    }

    if (self.$numeric_dict[hash] !== undefined && $B.rich_comp("__eq__", key, hash)) {
      return true;
    }
  ...
  dict.__eq__ = function () {
    ...
    for (var k in self.$numeric_dict) {
      if (other.$numeric_dict.hasOwnProperty(k)) {
        if (!$B.rich_comp("__eq__", other.$numeric_dict[k][0], self.$numeric_dict[k][0])) {
          return false;
        }
        ...
        for (var i = 0, len = pairs.length; i < len; i++) {
          if ($B.rich_comp("__eq__", k, pairs[i][0]) && $B.rich_comp("__eq__", self.$numeric_dict[k], pairs[i][1])) {
            flag = true;
            break;
          }
        }
    ...
    for (var k in self.$string_dict) {
      if (!other.$string_dict.hasOwnProperty(k) || !$B.rich_comp("__eq__", other.$string_dict[k][0], self.$string_dict[k][0])) {
        return false;
      }
    }
    ...
    for (var hash in self.$object_dict) {
      ...
      for (var i = 0, len_i = pairs.length; i < len_i; i++) {
        ...
        for (var j = 0, len_j = other_pairs.length; j < len_j; j++) {
          if ($B.rich_comp("__eq__", key, other_pairs[j][0]) && $B.rich_comp("__eq__", value, other_pairs[j][1][0])) {
            flag = true;
            break;
          }
        }
  ...
  dict.__init__ = function (self, first, second) {
    ...
    if (args.length > 1) {
      throw _b_.TypeError.$factory("dict expected at most 1 argument" + ", got 2");
    } else if (args.length == 1) {
      args = args[0];

      if (args.__class__ === dict) {
        ['$string_dict', '$str_hash', '$numeric_dict', '$object_dict'].forEach(function (d) {
          for (key in args[d]) {
  ...
  dict.fromkeys = function () {
    ...
    while (1) {
      try {
        ...
        if (klass === dict) {
          dict.$setitem(res, key, value);
        } else {
          $B.$getattr(res, "__setitem__")(key, value);
        }
      } catch (err) {
      ...
      if (isinstance(o, dict)) {
        ...
        while (true) {
          ...
          try {
            ...
            throw _b_.TypeError.$factory("cannot convert dictionary" + " update sequence element #" + i + " to a sequence");
          }

          if (key_value.length !== 2) {
            throw _b_.ValueError.$factory("dictionary update " + "sequence element #" + i + " has length " + key_value.length + "; 2 is required");
          }
...
(function ($B) {
  ...
  $B.$isNodeList = function (nodes) {
    try {
      ...
      return _typeof(nodes) === "object" && re.exec(result) !== null && nodes.length !== undefined && (nodes.length == 0 || _typeof(nodes[0]) === "object" && nodes[0].nodeType > 0);
    } catch (err) {
      return false;
    }
  };
  ...
  DOMEvent.__getattribute__ = function (self, attr) {
    switch (attr) {
      ...
      case 'svgX':
        ...
        throw _b_.AttributeError.$factory("event target is not an SVG " + "element");
  ...
  var Options = {
    __class__: _b_.type,
    __delitem__: function __delitem__(self, arg) {
    ...
    __mro__: [object],
    __setattr__: function __setattr__(self, attr, value) {
      self.parent.options[attr] = value;
    },
    __setitem__: function __setitem__(self, attr, value) {
      self.parent.options[attr] = $B.$JS2Py(value);
    },
    __str__: function __str__(self) {
      return "<object Options wraps " + self.parent.options + ">";
    },
    append: function append(self, element) {
      self.parent.options.add(element.elt);
    },
    insert: function insert(self, index, element) {
      if (index === undefined) {
        self.parent.options.add(element.elt);
      } else {
    ...
    item: function item(self, index) {
      return self.parent.options.item(index);
    },
    namedItem: function namedItem(self, name) {
      return self.parent.options.namedItem(name);
    },
    remove: function remove(self, arg) {
      self.parent.options.remove(arg.elt);
    },
    $infos: {
      __module__: "<pydom>",
  ...
  DOMNode.$factory = function (elt, fromtag) {
    ...
    if (typeof elt == "number" || typeof elt == "boolean" || typeof elt == "string") {
      return elt;
    }

    if (fromtag === undefined) {
      if (DOMNode.tags !== undefined) {
  ...
  DOMNode.__add__ = function (self, other) {
    ...
    if (_b_.isinstance(other, TagSum)) {
      ...
      try {
        ...
        throw _b_.TypeError.$factory("can't add '" + $B.class_name(other) + "' object to DOMNode instance");
      }
    }
  ...
  DOMNode.__dir__ = function (self) {
    ...
    for (var attr in DOMNode) {
      if (attr.charAt(0) != "$" && res.indexOf(attr) == -1) {
        res.push(attr);
      }
    }
  ...
  DOMNode.__getattribute__ = function (self, attr) {
    ...
    switch (attr) {
      ...
      case "width":
        ...
        if (self.elt.style[attr]) {
          ...
          throw _b_.AttributeError.$factory("style." + attr + " is not set for " + _b_.str.$factory(self));
        }
    ...
    if (attr == "select" && self.elt.nodeType == 1 && ["INPUT", "TEXTAREA"].indexOf(self.elt.tagName.toUpperCase()) > -1) {
    ...
    if (res !== undefined) {
      ...
      if (attr == 'options') {
        return Options.$factory(self.elt);
      }

      if (attr == 'style') {
        return $B.JSObject.$factory(self.elt[attr]);
      }
  ...
  DOMNode.__getitem__ = function (self, key) {
    if (self.elt.nodeType == 9) {
      ...
      if ((typeof key == "number" || typeof key == "boolean") && typeof self.elt.item == "function") {
        var key_to_int = _b_.int.$factory(key);

        if (key_to_int < 0) {
          key_to_int += self.elt.length;
        }
        ...
        return res;
      } else if (typeof key == "string" && self.elt.attributes && typeof self.elt.attributes.getNamedItem == "function") {
  ...
  DOMNode.__hash__ = function (self) {
    return self.__hashvalue__ === undefined ? self.__hashvalue__ = $B.$py_next_hash-- : self.__hashvalue__;
  };
      ...
      switch (attr) {
        ...
        case "height":
          if (_b_.isinstance(value, _b_.int) && self.elt.nodeType == 1) {
            ...
            throw _b_.ValueError.$factory(attr + " value should be" + " an integer, not " + $B.class_name(value));
          }
      ...
      while (!!proto && proto !== Object.prototype && nb++ < 10) {
        ...
        if (!!descriptors && typeof descriptors.hasOwnProperty == "function") {
          if (descriptors.hasOwnProperty(attr)) {
            if (!descriptors[attr].writable && descriptors[attr].set === undefined) {
      ...
      return function (ev) {
        try {
          ...
          if (err.__class__ !== undefined) {
            ...
            try {
              $B.$getattr($B.stderr, "write")(msg);
            } catch (err) {
              console.log(msg);
            }
          } else {
            try {
              $B.$getattr($B.stderr, "write")(err);
            } catch (err1) {
              console.log(err);
            }
          }
  ...
  DOMNode.Class = function (self) {
    if (self.elt.className !== undefined) {
      return self.elt.className;
    }
  ...
  DOMNode.getContext = function (self) {
    ...
    var obj = self.elt;
    return function (ctx) {
      return JSObject.$factory(obj.getContext(ctx));
    };
  };
  ...
  DOMNode.set_style = function (self, style) {
    if (!_b_.isinstance(style, _b_.dict)) {
      throw _b_.TypeError.$factory("style must be dict, not " + $B.class_name(style));
    }
  ...
  DOMNode.set_value = function (self, value) {
    self.elt.value = _b_.str.$factory(value);
  };
...
(function ($B) {
  ...
  function rstrip(s, strip_chars) {
    ...
    while (nstrip < len && _chars.indexOf(s.charAt(len - 1 - nstrip)) > -1) {
      nstrip++;
    }
  ...
  function make_node(top_node, node) {
    ...
    if (node.C.$genjs) {
      ...
      ctx_js = node.C.$genjs = node.C.to_js();
    }
    ...
    if (ctx_js) {
      ...
      if (ctype == "yield") {
        ...
        var js = "var sent_value = this.sent_value === undefined ? " + "None : this.sent_value;",
            h = "\n" + ' '.repeat(node.indent);
        js += h + "this.sent_value = None";
        js += h + "if(sent_value.__class__ === $B.$GeneratorSendError)" + "{sent_value.err.$stack.splice(0, 0, $B.freeze([$top_frame])[0]);" + " throw sent_value.err};";
  ...
  $B.genNode = function (data, parent) {
    ...
    this.addChild = function (child) {
      if (child === undefined) {
        console.log("child of " + this + " undefined");
      }
    ...
    this.clone_tree = function (exit_node, head) {
      ...
      if (head && (this.is_break || this.is_continue)) {
        ...
        res.data += 'var err = new Error("break"); ' + "err.__class__ = $B.GeneratorBreak; throw err;";
    ...
    this.src = function (indent) {
      indent = indent || 0;
      var res = [this.indent_src(indent) + this.data],
          pos = 1;
  ...
  function make_next(self, yield_node_id) {
    ...
    js = 'var $top_frame = ["' + self.iter_id + '",$locals,"' + self.module + '",$locals_' + self.module.replace(/\./g, '_') + '];' + '$B.frames_stack.push($top_frame); var $stack_length = ' + '$B.frames_stack.length;';
    fnode.addChild(new $B.genNode(js));
    ...
    while (1) {
      ...
      if (exit_node.loop_start !== undefined) {
        ...
        while (start < exit_parent.children.length && (exit_parent.children[start].is_except || exit_parent.children[start].is_else)) {
          start++;
        }
      ...
      if ((has_break || has_continue) && rest.length > 0) {
        ...
        var catch_test = "catch(err)" + "{if(err.__class__ !== $B.GeneratorBreak){throw err}}";
  ...
  generator.__enter__ = function (self) {
    console.log("generator.__enter__ called");
  };

  generator.__exit__ = function (self) {
    console.log("generator.__exit__ called");
  };
  ...
  generator.$$throw = function (self, type, value, traceback) {
    ...
    if (exc.$is_class) {
      if (!_b_.issubclass(type, _b_.BaseException)) {
        throw _b_.TypeError.$factory("exception value must be an " + "instance of BaseException");
...
(function ($B) {
  ...
  var browser = {
    ...
    bind: function bind() {
      ...
      if (typeof options == "boolean") {} else if (options.__class__ === _b_.dict) {
      ...
      return function (callback) {
        if ($.elt.__class__ && _b_.issubclass($.elt.__class__, $B.JSObject)) {
          var f = function f(ev) {
            try {
  ...
  modules['javascript'] = {
    ...
    JSConstructor: {
      __get__: function __get__() {
        console.warn('"javascript.JSConstructor" is deprecrated. ' + 'Use window.<js constructor name>.new() instead.');
    ...
    JSObject: {
      __get__: function __get__() {
        console.warn('"javascript.JSObject" is deprecrated. To use ' + 'a Javascript object, use window.<object name> instead.');
        return $B.JSObject;
      },
      __set__: function __set__() {
        throw _b_.AttributeError.$factory("read only");
  ...
  modules['_sys'] = {
    Getframe: function Getframe() {
      var $ = $B.args("_getframe", 1, {
        depth: null
      }, ['depth'], arguments, {
        depth: 0
      }, null, null),
          depth = $.depth;
      return $B._frame.$factory($B.frames_stack, $B.frames_stack.length - depth - 1);
    },
    exc_info: function exc_info() {
      for (var i = $B.frames_stack.length - 1; i >= 0; i--) {
        var frame = $B.frames_stack[i],
    ...
    excepthook: function excepthook(exc_class, exc_value, traceback) {
    ...
    settrace: function settrace() {
      var $ = $B.args("settrace", 1, {
        tracefunc: null
      }, ['tracefunc'], arguments, {}, null, null);
  ...
  $B.cell.cell_contents = $B.$call(_b_.property)(function (self) {
...
},{}]},{},["../node_modules/parcel-bundler/src/builtins/hmr-runtime.js","brython.js"], null)
//# sourceMappingURL=/brython.974647d8.js.map